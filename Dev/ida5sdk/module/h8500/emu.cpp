...
static void trace_sp(void)
{
  ...
  {
    int size = 2;
    if ( cmd.Op2.type == o_reglist ) size *= calc_reglist_count(cmd.Op2.reg);
    add_stkpnt(size);
    return;
  }
  ...
  {
    int size = 2;
    if ( cmd.Op1.type == o_reglist ) size *= calc_reglist_count(cmd.Op1.reg);
    add_stkpnt(-size);
    return;
  }
  // xxx @--sp
...
//----------------------------------------------------------------------
int h8500_get_frame_retsize(func_t *pfn)
{
