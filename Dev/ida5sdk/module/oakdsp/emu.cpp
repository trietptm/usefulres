...
//      if ( !isload ) interr("emu2");
    ...
    case o_local: // local variables
      if ( may_create_stkvars() )
      {
         ...
         if ( (pfn != NULL) && (pfn->flags & FUNC_FRAME) && ua_stkvar(x, x.addr) )
...
static void trace_sp(void)
{
  ...
  switch (cmd.itype)
  {
    ...
    case OAK_Dsp_rets:
      add_auto_stkpnt(cmd.ea+cmd.size, 1 + cmd.Op1.value);
      break;
    ...
    case OAK_Dsp_addv:
            ...
            {
                    frame = cmd.Op1.value;
                    add_auto_stkpnt(cmd.ea + cmd.size, frame );
            }
      break;
...
//----------------------------------------------------------------------
int OAK_get_frame_retsize(func_t * /*pfn*/)
{
