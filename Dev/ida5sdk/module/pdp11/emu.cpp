...
void loadR0data(op_t *x, int sme)
{
  ...
  if(cmd.Op1.type == o_imm) {
    if(cmd.itype == pdp_mov) {
      ...
      if(!sme) emuR0data.b[0] = (ushort)cmd.Op1.value;
      else     emuR0data.b[1] = (ushort)cmd.Op1.value;
      return;
    }
    if(!cmd.bytecmd) goto undefdat;
undefbyt:
  ...
  case o_reg:        // 0
      if(x.reg == rR0) {
        if(cmd.Op2.type == o_void) // one operand cmd
          if(cmd.itype != pdp_clr) goto undefall;
          else {
