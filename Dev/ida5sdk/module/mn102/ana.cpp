...
static ulong near LoadData(int bytes)
{
ulong dt=0;
int i;
        // загрузим данные
        for(i=0;i<bytes;i++){
                ulong nb=ua_next_byte();
                dt|=nb<<(8*i);
        }
        return(dt);
}
...
static void near SetLabel(op_t &op, int bytes)
{
ulong off;
//        char cc[20];
        op.type=o_near;
...
//        sprintf(cc,"%lx",off);
//        msg(cc);
        op.addr=op.value=(off+(ulong)cmd.size+cmd.ea) & 0xFFFFFFL;
...
static void near SetImm(op_t &op, int bytes)
{
...
static void near SetImmC(op_t &op, int Val)
{
  op.type=o_imm;                        // непосредственное
  op.dtyp=dt_dword;                     // реально - не совсем байт
...
// адресация регистром
static void near SetReg(op_t &op,uchar Reg)
{
        op.type=o_reg;
        op.reg=Reg;
        op.addr=op.value=0;
...
// косвенная адресация
static void near SetRReg(op_t &op,uchar Reg)
{
        op.type=o_reg;
...
static void near SetDisplI(op_t &op,uchar Reg, uchar RegI)
{
        op.type=o_reg;
...
// косвенная адресация с доп. смещением
static void near SetDispl(op_t &op,uchar Reg, int OffSize)
{
        op.type=o_displ;
		op.dtyp=dt_dword;                     // реально - не совсем байт
        op.reg=Reg;
...
static void near SetMem(op_t &op, int AddrSize, int DataSize)
{
        op.type=o_mem;
