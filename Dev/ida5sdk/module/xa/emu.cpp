...
static void handle_operand(op_t &x,int loading)
{
  switch ( x.type )
  {
	...
	if (check_insn(1, XA_mov, o_reg, reg+rR0, o_imm, BADADDR))
        {
          ...
          ua_dodata(v, dtyp);
          if ( !loading ) doVar(v);     // write access
	...
	if (auto_state == AU_USED &&
          isEnabled(ea-1) && isUnknown(get_flags_novalue(ea-1)) &&
          get_byte(ea-1) == 0 && cmd.ea != ea-2)
        {
          auto_make_code(ea-1);
        }
...
// mov.b R1H, #0
    ...
    case XA_pop:
        ...
        (cmd.Op1.phrase == fRlistL || cmd.Op1.phrase == fRlistH))
      {
        ...
        if (bits && should_trace_sp() && pfn && !get_sp_delta(pfn, cmd.ea))
          add_auto_stkpnt(cmd.ea, (cmd.itype==XA_push)?-2*bits:2*bits);
        ...
        func_t *pfn = get_func(cmd.ea);
        if (should_trace_sp() && pfn && !get_sp_delta(pfn, cmd.ea))
          add_auto_stkpnt(cmd.ea, (cmd.itype==XA_push)?-2:2);
      } else {
   ...
   case XA_adds:
     ...
     if (cmd.Op1.type == o_reg && cmd.Op1.reg == rR7)
     {
       if (cmd.Op2.type == o_imm)
       {
         ...
         if (should_trace_sp() && pfn && !get_sp_delta(pfn, cmd.ea))
           add_auto_stkpnt(cmd.ea, (cmd.itype==XA_sub)?-offset:offset);
       } else {
     ...
     if (cmd.Op1.type == o_reg && cmd.Op1.reg == rR7)
     {
         ...
         cmd.Op2.indreg == rR7)
       {
         ...
         if (pfn && !get_sp_delta(pfn, cmd.ea))
           add_auto_stkpnt(cmd.ea, cmd.Op2.addr);
       } else {
...
bool xa_create_func(func_t *pfn)
{
   ea_t prologue = pfn->startEA;
   int more;
   int prologue_at_end = 0;

   if (pfn->frame && pfn->frsize)
   {
     add_frame(pfn, pfn->frsize, pfn->frregs, 0);
     return 1;
   }
   if (ua_code(prologue))
   {
   ...
   add_frame(pfn, pfn->frsize, pfn->frregs, 0);
   if (prologue_at_end)
     add_auto_stkpnt(pfn->startEA, -pfn->frsize-pfn->frregs);

   return 1;
}

bool xa_is_switch(switch_info_t *si)
{
  int value = 0;
  int prev;
  ...
  if (cmd.Op1.type == o_phrase && cmd.Op1.phrase == fRi)
  {
    ...
    if (value)
    {
      if (get_byte(si->jumps + 2*si->ncases) == 0xfe)
        ua_add_cref(saved.ea, si->jumps + 2*si->ncases, fl_F);
    }
    cmd = saved;
  }
  return value;
...
int xa_frame_retsize(func_t *pfn)
{
  return pfn->is_far() ? 2 : 4;
}

void xa_stkvar_def(char *buf, size_t bufsize, const member_t *mptr, sval_t v)
{
  char sign = '+';
  if ( v < 0 )
  {
