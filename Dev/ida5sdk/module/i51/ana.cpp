...
//----------------------------------------------------------------------
inline ulong get_next_24bits()
{
  ulong high = ua_next_byte();
  ulong low  = ua_next_word();
  return low | (high<<16);
}
...
//----------------------------------------------------------------------
// register indirect
static int op_ph(op_t &x, int reg, int dtyp)
{
  if ( reg >= rDR32 && reg <= rDR52 ) return 0;
  x.type = o_phrase;
  x.dtyp = dtyp;
...
static int op_ds(op_t &x, int phrase, uval_t disp, int dtyp)
{
...
//----------------------------------------------------------------------
static ulong truncate(uval_t addr)
{
  if ( ptype == prc_51 )
    return addr & 0xFFFF;
  else
...
//----------------------------------------------------------------------
static int make_short(int itype, uchar b)
{
  cmd.itype = itype;
  static uchar bregs[] = { rR0, rWR0, 0, rDR0 };
  static int dtyps[] = { dt_byte, dt_word, dt_dword };
  int idx = (b >> 2) & 3;
...
static int ana_basic(void)
{
  ...
  if ( nibble0 < 4 )              // complex coding, misc instructions
  {
    switch ( nibble0 )
    {
      case 0:
        {
          static char misc0[16] ={I51_nop, I51_jbc, I51_jb,  I51_jnb,
      ...
      case 2:
        {
          static char misc2[16] ={I51_ljmp,I51_lcall,I51_ret,I51_reti,
      ...
      case 3:
        {
          static char misc3[16] ={I51_rr,  I51_rrc, I51_rl,  I51_rlc,
    ...
    static char regulars[16] = {I51_inc, I51_dec, I51_add, I51_addc,
