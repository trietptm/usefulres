    ...
    case processor_t::newfile:
      {
        ...
        if( sptr != NULL )
        {
          if( sptr->startEA - get_segm_base( sptr ) == 0 )
          {
            ...
            {
              ulong ea = sptr->startEA + entries[i].off;
              if( isEnabled(ea) )
              {
                doWord( ea, 2 );
...
//                set_op_type( ea, offflag(), 0 );
                ...
                ulong ea1 = sptr->startEA + get_word( ea );
                auto_make_proc( ea1 );
...
static char *shnames[] = { "Z8", NULL };
static char *lnames[]  = { "Zilog Z8 MCU", NULL };
