...
static void process_operand(op_t &op, int use)
{
  switch ( op.type )
  {
    ...
    case o_reg:
      ...
      {
        if ( may_create_stkvars() && get_func(cmd.ea) != NULL && ua_stkvar(op, 2 * op.value))
...
static void trace_sp(void)
{
  switch (cmd.itype)
  {
    case TMS320C55_pop1: // pop dst; pop dbl(ACx); pop Smem; pop dbl(Lmem)
      add_auto_stkpnt(cmd.ea+cmd.size, (cmd.Op1.tms_operator1 & TMS_OPERATOR_DBL) ? 4:2);
      break;
    case TMS320C55_pop2: // pop dst1, dst2; pop dst, Smem
      add_auto_stkpnt(cmd.ea+cmd.size, 4);
      break;
    ...
    case TMS320C55_pshboth:
      ...
      break;
    case TMS320C55_reti:
      add_auto_stkpnt(cmd.ea+cmd.size, 6);
      break;
    case TMS320C55_aadd:
