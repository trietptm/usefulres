...
static char cmt01[] = "Timer overflow";
static char cmt02[] = "A/D conversion complete";
static char cmt03[] = "HSI data available";
static char cmt04[] = "High speed output";
static char cmt05[] = "HSI.0";
static char cmt06[] = "Software timer";
static char cmt07[] = "Serial port";
static char cmt08[] = "EXTINT";
static char cmt09[] = "reserved (FF)";
static char cmt10[] = "Transmit";
static char cmt11[] = "Receive";
static char cmt12[] = "HSI FIFO 4";
static char cmt13[] = "Timer 2 capture";
static char cmt14[] = "Timer 2 overflow";
static char cmt15[] = "EXTINT1";
static char cmt16[] = "HSI FIFO FULL";
...
static char *RegNames[] = { "cs", "ds", "WSR", "WSR1" };
int extended = 0;

//----------------------------------------------------------------------
//dash: это тоже оказалось не нужно

//static long near specialSeg( ushort sel )
//{
        ...
        for( int i = 0; i < qnumber(entries); i++ )
        {
          ...
          if( isEnabled(ea) )
          {
            switch( entries[i].type )
            {
              ...
              case I196F_CMT:
                if( entries[i].cmt )
                  add_long_cmt( ea, 1, entries[i].cmt );
                else
                  describe( ea, 1, "" );
                break;
...
//      ulong bottom = toEA( inf.baseaddr, 0 );
...
// dash: дырку искать не пришлось, но я оставил это как пример на будущее
        ...
        {
          ulong ea = toEA( inf.baseaddr, ptr->addr );
          ulong oldea = get_name_ea( BADADDR, ptr->name );
          if( oldea != ea )
          {
...
//--------------------------------------------------------------------------
static char *shnames[] = { "80196", "80196NP", NULL };
static char *lnames[]  = { "Intel 80196", "Intel 80196NP", NULL };
...
//------------------------------------------------------------------------
static bool can_have_type(op_t &x)      // returns 1 - operand can have
{
  switch ( x.type )
  {
    case o_void:
