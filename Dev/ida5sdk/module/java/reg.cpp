...
static void sm_validate(SegInfo *si)
{
  ...
  do {
    ...
    if(!isHead(get_flags_novalue((ea_t)nid))) {
      ...
      msg("StackMap refers to nonHead offset %lX in Method#%u\n",
          (ulong)((ea_t)nid - si->startEA), si->id.Number);
    }
...
// Otherwise the code should be returned to the caller:
  ...
  switch(msgid) {
    ...
    case processor_t::rename:
      va_argi(va, ea_t);
      for(char const *pn, *p = va_argi(va, const char *);
...
inv_name:
        ...
        for(int i = 0; i < 4; i++) if(!isxdigit((uchar)*++pn)) goto inv_name;
      }
      break;
    ...
    case processor_t::newprc:
      if(va_argi(va, int) == 1) {   // debug mode
    ...
    case processor_t::loader:
      {
        linput_t *li = va_argi(va, linput_t *);
    ...
    case processor_t::out_src_file_lnnum:
      if(jasmin()) {
        va_argi(va, const char *); // skip file name
        printf_line(2, COLSTR(".line %u", SCOLOR_ASMDIR), va_argi(va, size_t));
        ++retcode;  // = 2
      }
      break;
    ...
    case processor_t::gen_asm_or_lst:
      {
        ...
        if(va_argi(va, bool)) {           // starting (else end of generation)
          va_argi(va, FILE *);            // output file (skip)
          bool isasm = va_argi(va, bool); // assembler-true, listing-false
          if(isasm && (idpflags & IDF_CONVERT)) {
            va_argi(va, int);              // flags of gen_file() (skip)
            *va_argi(va, gen_outline_t**) = out_asm_file;
            idpflags |= IDM_OUTASM;
          }
    ...
    case processor_t::get_autocmt:
      {
        char *buf = va_argi(va, char *);
        if(make_locvar_cmt(buf, va_argi(va, size_t))) ++retcode; // = 2
      }
      break;
...
static int j_realcvt(void *m, eNE e, ushort swt)
{
  inf.mf = 0;
...
void check_float_const(ea_t ea, void *m, char len)
{
  if(!has_cmt(get_flags_novalue(ea)) && j_realcvt(m, NULL, (uchar)len) < 0) {
    ...
    *p++ = '0';
    *p++ = 'x';
    do p += qsnprintf(p, 5, "%04X", ((ushort *)m)[len]); while(--len >= 0);
    QueueMark(Q_att, ea);
...
static const char *set_idp_options(const char *keyword, int value_type,
                                   const void *value)
{
...
"\n";
  ...
  if(!keyword) {
    ulong tmp = (idpflags >> 16) & IDM__REQMASK, old = idpflags;
