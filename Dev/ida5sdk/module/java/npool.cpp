...
void myBase(const char *arg)
{
  ...
  if(!arg) {  // load old file
    ...
    register int v;
    i = sizeof(curClass);
    switch(v = ConstantNode.altval(CNA_VERSION)) {
      default:
    ...
    if(   i != (int)ConstantNode.supval(CNS_CLASS, NULL, (size_t)-1)
       || ConstantNode.supval(CNS_CLASS, &curClass, sizeof(curClass)) <
                                          (long)offsetof(ClassInfo, MajVers)
...
badbase:
    ...
    if((i = strlen((char *)arg)) >= (sizeof(str))) {
...
badindex:
          error("Bad record in constant pool.\n"
                ...
                "(maxnum %u, file offset after read 0x%lX)",
...
badname:
    ...
    if(   (loadMode & MLD_EXTREF)
      ...
      for(j = 1; j <= curClass.xtrnCnt; j++)
        if(!CmpString(co->_name, (ushort)(rfc = XtrnNode.altval(j)))) {
...
static void setPoolReference(void)
{
  ...
  for(ui = 0, i = 1; (ushort)i <= curClass.xtrnCnt; i++) {
    ...
    for( ; (ushort)ii <= curClass.xtrnCnt; ii++) {
      if((j = XtrnNode.altval(ii, '0')) == 0) continue;
  ...
  if((curClass.xtrnCnt = (ushort)ui) != 0) {
    ...
    XtrnNode.kill();
    curClass.xtrnNode = 0;
    del_segm(curClass.xtrnEA, SEGDEL_PERM);
    curClass.xtrnEA = 0;
  }
  msg("OK\n");
}
...
#error
          ...
          if(   (co.flag & HAS_FLDNAME)
             && mask
             && (j = attrStr(i, ARQ_CHECK, str)) <= a_LocalVariables
             && (mask & (1 << j))) mask ^= (1 << j);
  ...
  if(k1) load_msg(emc, "unused", pass ? "(except Class/Type/String)" :
                                        "NameAndType", k1);
  if(k2) {
    if(k3) {
      load_msg(emc, pass ? "unnamed" : "unrefernced",
                    pass ? "Class" : "Utf8", k3);
      k2 -= k3;
    }
...
static void ValidateStoreLocVar(ushort slot, LocVar & lv)
{
  ...
  lv.ScopeTop = (ushort)(id = (ulong)lv.ScopeBeg + lv.ScopeTop);
...
#error
    ...
    if(!dble && opis._Sopstr == (1 | (_OP_UTF8_ << 16))) {
      ...
      if(ConstantNode.supval((ulong)lv.var.Dscr << 16, tmp, sizeof(tmp),
...
//-----------------------------------------------------------------------
static uchar *annot_elm(uchar *ptr, ulong *psize, uchar is_array=0);
static uchar *annotation(uchar *p, ulong *psize)
{
  if(*psize < 2) {
bad:
    return(NULL);
...
//---------------------------------------------------------------------------
static uchar *annot_elm(uchar *ptr, ulong *psize, uchar is_array)
{
  if(*psize < 1+2) {
bad:
    return(NULL);
...
//-----------------------------------------------------------------------
static bool sm_chkargs(uchar **pptr, ulong *pDopSize, ushort cnt)
{
  register union {
    uchar   *p1;
    ushort  *p2;
...
//-----------------------------------------------------------------------
static int sm_load(ushort declcnt, ulong *pDopSize)
{
  register union {
    uchar   *p1;
    ushort  *p2;
  ...
  *p2++ = declcnt;  // counter
  ...
  do {
    ...
    }
    {
      ...
      if((ulong)off >= curSeg.CodeSize) goto declerr_w;
      prevoff = off;
    ...
    switch(attrStr(k, mode, &atrs[1])) {
      case a_SourceDebugExtension:
        ...
        if(dopsize < 0x10000) {
          ...
          ulong pos = qftell(myFile), ssz = FileSize;
...
declerr:
          ++fat;
declerr_w:
          (fat ? error : load_msg)("Illegal declaration of %s%s",
        ...
        switch(opis.type) {
          ...
          default:
            ...
            temp.altset(++r, (((ulong)k) << 16) | 0xFFFF);
            break;
        }
        temp.altset(0, r);
        break;
...
duplerr_w:
      ...
      case a_Exceptions:
        ...
        dopsize -= 2;
        if((dopsize / 2) != (ulong)read2()) goto declerr_w;
        dopsize /= 2;
        temp.create();
        curSeg.thrNode = temp;
        r = 0;
      ...
      case a_LineNumberTable:
        ...
        dopsize -= 2;
        if((r /= 4) != (ulong)read2()) goto declerr_w;
        if(!dopsize) {
//Symantec error (strip) #1
      ...
      case a_LocalVariableTable:
        ...
        dopsize -= 2;
        if((r /= 10) != (ulong)read2()) goto declerr_w;
        while(r--) {
          LocVar  lv;
    ...
    || sizeof(lv.var.Dscr)*2 != sizeof(ulong)
#error
#endif
#endif
          *(ulong *)&lv.var.Dscr &= 0;
...
//Microsoft VisualJ++ error (purge?)
      ...
      case a_InnerClasses:
        ...
        dopsize -= 2;
        if((r /= 8) != (ulong)read2()) goto declerr_w;
        if(!r) {
        ...
        while(dopsize--) {
          ...
          *(uchar *)&k = 0;
          ...
          ic.access = read2();
          if((r = ic.access & ACC_ACCESS_MASK) & (r-1))     k |= 0x101;
          if(!(uchar)k) {
...
badvers:
  ...
  if(curClass.MajVers >= JDK_SMF_MAJOR_MIN) SMF_mode = 1;
  else if(curClass.JDKsubver == 3) {  // jdk1.3 never see
untested:
    ask_for_feedback(
...
//---
  for(i = 1; (ushort)i <= curClass.FieldCnt; i++) {
    ...
    if(   ((curField.id.access = read2()) & ~ACC_FIELD_MASK)
       || ((j = curField.id.access & ACC_ACCESS_MASK) & (j-1))) {
...
//      curField.id.extflg |= EFL_ACCESS;
    ...
    for(int n = 0; n < qnumber(curField.annNodes); n++)
      if(curField.annNodes[i] == (uval_t)-1) ++curField.annNodes[i];
...
//--
  ...
  for(i = 1; (ushort)i <= curClass.MethodCnt; i++) {
    ...
    j = curSeg.id.access = read2();
    if(!(j & ~ACC_METHOD_MASK) && !((j &= ACC_ACCESS_MASK) & (j-1))) j &= 0;
    else {
...
//    if(curSeg.id.extflg & EFL_ACCESS)
    ...
    for(int n = 0; n < qnumber(curSeg.annNodes); n++)
      if(curSeg.annNodes[i] == (uval_t)-1) ++curSeg.annNodes[i];
...
//--
  ...
  for(int n = 0; n < qnumber(curClass.annNodes); n++)
    if(curClass.annNodes[i] == (uval_t)-1) ++curClass.annNodes[i];
  myFile = NULL;
  if(curClass.encClass == 0xFFFF) ++curClass.encClass;  // unification in out
  if(FileSize)
    warning("This file has extra information (pos=0x%lx)", qftell(fp));
//---
...
//--
  ...
  for(i = 1; (ushort)i <= curClass.MethodCnt; i++) {
    ...
    else do if((sz = ua_code(ea)) == 0) ++sz; while((ea += sz) < end);
