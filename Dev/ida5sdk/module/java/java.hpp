...
idaman void ida_export verror(const char *message, va_list va) __attribute__((noreturn));
#else
idaman void ida_export __declspec(noreturn) verror(const char *message, va_list va);
void __declspec(noreturn) error(const char *message, ...);
#endif
#include "../idaidp.hpp"
#include <fpro.h>
...
typedef struct {
                ...
                ulong   maxSMsize;      // optimize memory allocation (StackMap)
...
// next fields added in JDK15 store-format
                ...
                ulong   maxAnnSz;       // optimize memory allocation (annotation)
                uval_t  genNode;        // list of stored generic attributes
               }ClassInfo;
...
int         print_loader_messages(char str[MAXSTR], const char *cmt);
...
#ifndef __GNUC__
#define _DCNR   __declspec(noreturn)
#define _GDCNR
#else
#define _DCNR
#define _GDCNR  __attribute__((noreturn))
#endif
_DCNR extern void  _destroyed(
#ifdef __debug__
                              const char *from
#else
                              void
...
_DCNR extern void  _faterr(uchar mode
#ifdef __debug__
                           , const char *from
#endif
                          )  _GDCNR;
...
struct sm_info_t {
  ulong     noff;   // start offset in blob
  ulong     eoff;   // end offset in blob
