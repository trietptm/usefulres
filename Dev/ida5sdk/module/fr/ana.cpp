...
#endif
        IDA_ASSERT(n == 16 || n == 32, "Invalid instruction size (!= 16 bits)");
    }
}

const struct opcode_t * opcode_t::find(int *_data) {
    IDA_ASSERT(_data != NULL, "argument is null in opcode_t::find()");
    ...
    for (int i = 0; i < qnumber(opcodes); i++) {
        ...
        switch (opcodes[i].opcode_size) {
            ...
            default:        IDA_ERROR("invalid opcode_size in ana_common()");
        }
...
static const int get_gr(const int num) {
    IDA_ASSERT(num >= 0 && num <= 15, "bad num in get_gr()");
    return num;
}

// get coprocessor register.
static const int get_cr(const int num) {
    IDA_ASSERT(num >= 0 && num <= 15, "bad num in get_cr()");
    return num + 16;
}

// get dedicated register.
static const int get_dr(const int num) {
    static int drs[] = {
        rTBR,
        rRP,
        rSSP,
    ...
    IDA_ASSERT(num >= 0 && num <= 15, "bad num in get_dr()");
    return drs[num];
}
...
static void set_imm(op_t &op, int imm, char d_typ) {
    ...
    switch (d_typ) {
        ...
        default:        IDA_ERROR("invalid d_typ in set_imm()");
    }
    op.dtyp = d_typ;
}
...
static void set_phrase(op_t &op, int type, int val, char d_typ) {
    switch (type) {
        ...
        default:
            IDA_ERROR("invalid type in set_phrase()");
    }
    op.type = o_phrase;
    op.specflag2 = type;
    op.dtyp = d_typ;
...
static void set_rel(op_t &op, int addr, char d_typ) {
    ...
    switch (d_typ) {    /* ugly but functional */
        ...
        default:
            IDA_ERROR("invalid type in set_rel()");
    }
...
static void swap_ops(op_t &o1, op_t &o2) {
    IDA_ASSERT(o1.type != o_void && o2.type != o_void, "one or both operands are undefined in swap_ops()");
    op_t tmp = o1;
    o1 = o2;
    o2 = tmp;
}
...
static void adjust_data(int size, int *data) {
    IDA_ASSERT(data != NULL, "argument is null in adjust_data()");
...
static void fill_op(int data, op_t &op, int operand, int operand_size, int flags) {
    ...
    switch (operand) {
        case O_gr:           // general register                         Ri
            IDA_ASSERT(operand_size == S_4, "O_gr with size != 4 bits");
            set_gr(op, data);
            break;

        case O_gri:          // general register indirect                @Ri
            IDA_ASSERT(operand_size == S_4, "O_gri with size != 4 bits");
            set_gri(op, data);
            break;
        ...
        case O_imm:          // immediate value                          #i
            {
                SWAP_IF_BYTE(data);
                if (cmd.itype == fr_enter)   data *= 4;
                if (cmd.itype == fr_addsp)   data = ((signed) data) * 4;
