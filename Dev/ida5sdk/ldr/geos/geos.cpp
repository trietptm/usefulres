...
//--------------------------------------------------------------------------
static netnode get_node(const GEOSappheader &ah, netnode *modnode, ushort libn)
{
  if ( libn >= ah.numlib )
  {
...
//--------------------------------------------------------------------------
static void describe_app(GEOSappheader &ah, ulong *segea)
{
  char buf[MAXSTR];
...
//--------------------------------------------------------------------------
static void bad_lib_reloc(int i, ushort off)
{
                              ...
                              const ulong *segea)
{
  ...
  {
    if ( segfix[i] == 0 ) continue;
    GEOSfixup *fix = (GEOSfixup *)qalloc(segfix[i]);
    if ( fix == NULL ) nomem("fix");
    ...
    for ( int j=0; j < n; j++ )
    {
      ea_t ea = get_segea(ah, segea, i) + fix[j].ofs;
      int ftype = fix[j].type & 0xF0;
      if ( ftype != 0x10 && ftype != 0x20 )
        ask_for_feedback("Unknown fixup type %02X", ftype);
      netnode libnode;
      if ( ftype == 0x10 ) libnode = get_node(ah,modnode,fix[j].type>>8);
      ushort w1 = get_word(ea);
      ea_t target;
      fixup_data_t fd;
      fd.type = FIXUP_SEG16;
                                   ...
                                   netnode *modnode)
{
  ...
  for ( int i=0; i < ah.numlib; i++ )
  {
    ...
    ea_t end = ea + nimps*4;
    add_segm(ea>>4, ea, end, libname, "XTRN");
    for ( x=n.alt1st(); x != BADNODE; x=n.altnxt(x),ea+=4 )
    {
      char buf[MAXSTR];
      qsnprintf(buf, sizeof(buf), "%s_%u", libname, ushort(x)-1);
      put_long(ea, 0xCB);
      ua_code(ea);
...
//--------------------------------------------------------------------------
void load_application(linput_t *li, long fpos, long fdelta)
{
  GEOSappheader ah;
  ...
  if ( ah.numlib != 0 )
  {
    lib = (GEOSliblist *)qalloc(ah.numlib*sizeof(GEOSliblist));
    if ( lib == NULL ) nomem("libs");
    lread(li, lib, ah.numlib*sizeof(GEOSliblist));
    modnode = (netnode *)qalloc(ah.numlib*sizeof(netnode));
    if ( modnode == NULL ) nomem("libnode");
    for ( int i=0; i < ah.numlib; i++ )
    {
      char libname[8+1];
      qstrncpy(libname, lib[i].name, sizeof(libname));
  ...
  if ( ah.numexp != 0 )
  {
    exp = (GEOSexplist *)qalloc(ah.numexp*sizeof(GEOSexplist));
    if ( exp == NULL ) nomem("exp");
  ...
  if ( ah.numseg != 0 )
  {
    ...
    if ( segd == NULL ) nomem("segd");
    lread(li, segd, ah.numseg*10);
    seglen = (ushort *)segd;
    segpos = (long   *)(seglen + ah.numseg);
    segfix = (ushort *)(segpos + ah.numseg);
    segflg = (ushort *)(segfix + ah.numseg);
    segea  = (ulong  *)(segflg + ah.numseg);
    ...
    for ( int i=0; i < ah.numseg; i++)
    {
      ...
      add_segm(ea>>4, ea, endea, NULL, (f & HF_CODE) ? "CODE" : "DATA");
      if ( found_data_segment ) ds_sel = find_selector(ea>>4);
...
static void show_geos2(GEOS2header &h)
{
  ...
  add_pgm_cmt("Protocol    : %u.%03u",
                          ...
                          h.protocol.rev);
  add_pgm_cmt("Flags       : %04X", h.flags);
  add_pgm_cmt("Password    : %.*s", sizeof(h.password), h.password);
  add_pgm_cmt("User info   : %s", geos2ibm(buf, h.info, sizeof(h.info)));
  add_pgm_cmt("Copyright   : %s", geos2ibm(buf, h._copyright, sizeof(h._copyright)));
}
