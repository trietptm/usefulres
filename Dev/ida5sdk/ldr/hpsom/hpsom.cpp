...
static long first_text_subspace_fpos = -1;
...
//--------------------------------------------------------------------------
inline char *get_space_name(linput_t *li, header &h, long nidx, char *buf, size_t bufsize)
{
  return get_name(li,
...
inline char *get_symbol_name(linput_t *li, header &h, long nidx, char *buf, size_t bufsize)
{
  return get_name(li,
...
//--------------------------------------------------------------------------
static void load_spaces(linput_t *li, header &h, long fpos, int n)
{
  if ( !n ) return;
  qlseek(li, fpos);
  char buf[MAXSTR];
...
static char *get_text_name(int nidx, char *buf, size_t bufsize)
{
  ...
  if ( nidx >= 0 && nidx < dl_ssize )
    ...
    qsnprintf(buf, bufsize, "0x%08lX", nidx);
  return buf;
}
