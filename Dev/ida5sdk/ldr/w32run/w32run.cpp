...
static void realize_relocation(void)
{
  ...
  ulong curv, maxv = 0, ost = wh.read_size - wh.reltbl_offset;
  ...
  for( ; ; ) {
    ...
    if(!cnt) break;
    if(ost < sizeof(long)) { first = -1; break; }
    ost -= sizeof(long);
    ...
    for( ; ; ) {
      if(offset > wh.reltbl_offset - 4) {
        ...
        ulong ea = minea + offset;
        showAddr(ea);
        if((curv = get_long(ea)) >= wh.mem_size) {
          msg("Doubtful value after relocation! (%lx=>%lx)\n", ea,
                                                            curv + minea);
          QueueMark(Q_att, ea);
  ...
  if(!ost && !first && maxv > wh.start_offset &&
     (maxv += minea) < topea)       set_segm_end(topea, maxv, 1);
}
...
{
  create_filename_cmt();
  add_pgm_cmt("Full size of allocation memmory: %0lXh", wh.mem_size);
...
//  inf.nametype = NM_EA4;
  ...
  minea = toEA(W32_DOS_LOAD_BASE, 0);
