...
// ...досюда порядок не менять! - mc68000
...
// names and features (ins.cpp) is accessed using this code.
  ...
  inline ulong get_canon_feature(void) const; // (see def in idp.hpp)
...
union value_u
{
  ...
  ushort v_short;
  ulong v_long;
  struct dq_t { ulong low; ulong high; } _dq;
...
idaman ulonglong ida_export ua_next_qword(void);
...
idaman int ida_export out_snprintf(const char *format, ...);
...
#define   OOFW_64       0x0050      //   32 bit width
...
idaman bool ida_export ua_stkvar(const op_t &x, adiff_t v);
...
idaman int ida_export ua_code(ea_t ea);
