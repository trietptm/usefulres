...
//          Although you will probably not destroy anything on
...
//          could lead to the exhauston of the netnode numbers (the numbering
...
// freely casted to 'ulong' and back. This makes it easy to store
...
// return 0.
  ...
  friend bool ida_export exist(const netnode &n)
...
//          as zeroes
  ...
  nodeidx_t altval(sval_t alt, char tag=atag) const
  {
...
//          0 - failed, element doesn't exist
  ...
  bool altdel(sval_t alt, char tag=atag)
  {
...
//      value - 8  bits
  ...
  uchar charval(sval_t alt, char tag) const         { return netnode_charval(*this, alt, tag); }
  bool charset(sval_t alt, uchar val, char tag)     { return supset(alt, &val, sizeof(val), tag); }
  bool chardel(sval_t alt, char tag)                { return supdel(alt, tag); }
...
// NB: do not use this function to retrieve strings, see supstr()!
  ...
  ssize_t supval(sval_t alt, void *buf, size_t bufsize, char tag=stag) const
...
// zero, then you can continue to use supval() instead of supstr()
  ...
  ssize_t supstr(sval_t alt, char *buf, size_t bufsize, char tag=stag) const
...
// returns: 1 - ok, 0 - error, should not occur
  ...
  bool supset(sval_t alt, const void *value, size_t length=0, char tag=stag)
...
// returns: 1 - ok, 0 - error, should not occur
  ...
  bool supdel(sval_t alt, char tag=stag)
...
// returns: number of bytes required to store a blob
  ...
  size_t blobsize(nodeidx_t start, char tag) // get size of blob
...
//          otherwise returns pointer to blob
  ...
  void *getblob(void *buf,     // buffer to receive (may be NULL, will be allocated)
                size_t *bufsize,// in: buffer size, out: blob size
                nodeidx_t start,// start index of blob
...
// returns: 1 - ok, 0 - error
  ...
  bool setblob(const void *buf,// blob to save
              size_t size,     // its size
              nodeidx_t start, // starting index
              char tag)        // tag of blob
...
// returns: number of copied/moved keys, BADNODE-failure, not enough memory
  ...
  nodeidx_t copyto(netnode target, nodeidx_t count=1) { return netnode_copy(netnodenumber, count, target.netnodenumber, false); }
  nodeidx_t moveto(netnode target, nodeidx_t count=1) { return netnode_copy(netnodenumber, count, target.netnodenumber, true); }
...
#define NNBASE_PAGE16  3        // 16-bit database
  ...
  static void upgrade(const char *oldbase, const char *newbase, void (*cb)(ulong));
...
// information about.
  ...
  string qsupval(nodeidx_t ea, char tag) const;
  void qsupset(nodeidx_t ea, const string &x, char tag);
...
// returns -1 - bad netlink type
  ...
  ssize_t name(char *buf, size_t bufsize);
