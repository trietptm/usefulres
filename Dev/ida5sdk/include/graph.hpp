 ...
 *
 *      Please note that this interface has not been settled yet
 *      and will be modified in the future versions!
 *
 *      The graph interface is known to be compatible with Borland CBuilder v6.0
 *      Due to the use of STL and virtual functions, it might be incompatible
...
//-------------------------------------------------------------------------
class intseq_t : public std::vector<int>
{
public:
  int  idaapi index(int value) const
  {
    const_iterator pos = std::find(begin(), end(), value);
    return pos == end() ? -1 : pos-begin();
  }
  bool idaapi contains(int value) const { return std::find(begin(), end(), value) != end(); }
...
typedef std::vector<intseq_t> array_of_intseq_t;
...
public:
  ...
  idaapi node_set_t(const abstract_graph_t *g);
...
class pointseq_t : public std::vector<point_t>
{
public:
  const char *idaapi dstr(void) const;
  size_t idaapi print(char *buf, size_t bufsize) const;
...
struct edge_info_t
{
  ...
  pointseq_t layout;
  void idaapi reserve_layout(void) { std::reverse(&layout[0], &layout[layout.size()]); }
...
typedef std::vector<row_info_t> graph_row_info_t;
...
class abstract_graph_t : public gdl_graph_t
{
  ...
  void idaapi depth_first(int root, class depth_first_info_t &di) const;
...
//  void breadth_first(node_ordering_t &breadth) const;
  ...
  // does a path from 'm' to 'n' exist?
  bool idaapi path(int m, int n) const { node_set_t v; return path(v, m, n); }
...
public:
  ...
  idaapi abstract_graph_t(void) : rect_edges_made(false), current_layout(layout_none) {}
  void idaapi clear(void);
  ...
  const rect_t &idaapi nrect(int n) const
    { return ((abstract_graph_t *)this)->nrect(n); }
  const edge_info_t *idaapi get_edge(edge_t e) const
    { return ((abstract_graph_t *)this)->get_edge(e); }
  ...
  void idaapi create_tree_layout(void);
  void idaapi create_circle_layout(point_t p, int radius);
};
...
class mutable_graph_t : public abstract_graph_t
{
  ...
  void idaapi restore_edges(int first_reserver_node);
...
public:
  ...
  typedef std::vector<rect_t> node_layout_t;
  node_layout_t nodes;
  edge_infos_t edges;

  idaapi mutable_graph_t(uval_t id) : gid(id) {}
  idaapi mutable_graph_t(const abstract_graph_t &g, uval_t id);
  size_t idaapi size(void) const { return succs.size(); }
  size_t idaapi node_qty(void) const;
  void idaapi clear(void);
...
#define COLLAPSED_NODE 0x80000000
  ...
  int  idaapi nsucc(int b) const  { return succs[b].size(); }
  int  idaapi npred(int b) const  { return preds[b].size(); }
  ...
  virtual void idaapi redo_layout(void);
  virtual void idaapi resize(int n);
  virtual int  idaapi add_node(const rect_t *r);
  virtual ssize_t idaapi del_node(int n);  // returns number of deleted edges
  virtual bool idaapi add_edge(int i, int j, const edge_info_t *ei);
  virtual bool idaapi del_edge(int i, int j);  // true: found and deleted the edge
  virtual bool idaapi replace_edge(int i, int j, int x, int y);
  virtual bool idaapi refresh(void);
#ifdef _MSC_VER // vc compiler is stupid
  virtual abstract_graph_t *idaapi clone(void) const;
#else
  virtual mutable_graph_t *idaapi clone(void) const;
#endif

  // get node rectangle
  const rect_t &idaapi nrect(int n) const
    { return ((mutable_graph_t *)this)->nrect(n); }
  rect_t &idaapi nrect(int n);
  virtual edge_info_t *idaapi get_edge(edge_t e);

  bool idaapi set_nrect(int n, const rect_t &r);
  bool idaapi set_edge(edge_t e, const edge_info_t *ei);

  void idaapi create_digraph_layout(void);
...
idaman void ida_export set_node_info(ea_t ea, int node, bgcolor_t *pcolor, ea_t *pea2, const char *text);
...
enum graph_notification_t
{
  ...
  grcode_clicked,             // a graph has been clicked
                              // in:  graph_viewer_t *gv
...
class TForm;
class graph_viewer_t;

inline graph_viewer_t *idaapi create_graph_viewer(TForm *parent, uval_t id)  { graph_viewer_t *gv = NULL; grentry(grcode_create_graph_viewer, parent, &gv, id); return gv; }
...
inline bool idaapi viewer_add_menu_item(graph_viewer_t *gv,
                                        ...
                                        void *ud,
                                        int flags)                           { return grentry(grcode_add_menu_item, gv, title, callback, ud, flags); }
...
inline void idaapi mutable_graph_t::create_digraph_layout(void)              {        grentry(grcode_create_digraph_layout, this); }
inline void idaapi abstract_graph_t::create_tree_layout(void)                {        grentry(grcode_create_tree_layout, this); }
inline void idaapi abstract_graph_t::create_circle_layout(point_t c, int radius) {    grentry(grcode_create_circle_layout, this, c.x, c.y, radius); }
...
inline size_t idaapi mutable_graph_t::node_qty(void) const                   { return grentry(grcode_node_qty, this); }
