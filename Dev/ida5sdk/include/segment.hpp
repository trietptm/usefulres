...
//      flags  - additional processing:
//                      ADDSEG_NOSREG - undefined all default segment registers
//                      ADDSEG_OR_DIE - don't return if can't add a segment
...
#define ADDSEG_NOSREG   0x0001  // set all default segment register values
                                // to BADSELs
#define ADDSEG_OR_DIE   0x0002  // exit() if can't add a segment
...
//      flags - combination of SEGDEL_... constants
...
inline segment_t *getseg(ea_t ea)
                                { return (segment_t *)(segs.get_area(ea)); }
...
//      perm   - meaningful only if the segment is being trimmed
//               (new end address is lower that the current end address)
//               0: don't disable addresses belonging to the segment
//                  if you want to create another segment, you need
//                  this setting
//               1: delete all information about trimmed bytes and
//                  disable addresses
...
idaman int ida_export set_segm_end(ea_t ea,ea_t newend,int perm); // move segment end
...
//      perm   - meaningful only if the segment is being shrinked
//               (new start address is lower that the current start address)
//               0: don't disable addresses belonging to the segment
//                  if you want to create another segment, you need
//                  this setting
//               1: delete all information about trimmed bytes and
...
idaman int ida_export set_segm_start(ea_t ea,ea_t newstart,int perm); // move segment start
...
// Convert a debugger segment to a normal segment and vice versa
//      s           - segment to modify
...
// returns true if the database contains only debugger segments
...
idaman bool ida_export set_segment_translations(ea_t segstart, ea_t *transmap);
...
//      name - new name, printf() style format string
...
idaman int ida_export vset_segm_name(segment_t *s,const char *name,va_list va);
inline int set_segm_name(segment_t *s,const char *name,...)
{
  va_list va;
  va_start(va, name);
  int code = vset_segm_name(s, name, va);
  va_end(va);
  return code;
}
...
idaman adiff_t ida_export segm_adjust_diff(segment_t *s, adiff_t delta);

// truncate an address depending on the segment
idaman ea_t ida_export segm_adjust_ea(segment_t *s, ea_t ea);
