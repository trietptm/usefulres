			...
			if(content)
			{
				...
				if(pLineText)
				{
					if(ch_inf_out && *ch_inf_out)
					{
						...
						char_inf* chInf1 = new char_inf[lineTextLen];
						if(chInf1)
						{
							memcpy(chInf1,pLineChInf,lineTextLen * sizeof(char_inf));
...
TCHAR *CPdfEngine::ExtractPageText(int pageNo, TCHAR *lineSep, RectI **coords_out, RenderTarget target, char_inf** ch_inf_out)
{
    ...
    TCHAR *result = ExtractPageText(page, lineSep, coords_out, target, false, ch_inf_out);
