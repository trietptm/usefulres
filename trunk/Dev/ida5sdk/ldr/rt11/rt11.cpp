...
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------
static void loadchunk(linput_t *li,ulong ea,ulong size,ulong base,long fpos,char *sclass)
{
  long p = qltell(li);
...
//
  ...
  if(topPrg & 1 || (ulong)topPrg > qlsize(li)) topPrg = 0;
  ...
  add_segm_ex(&s, "asect", "DATA", ADDSEG_NOSREG);
...
nons:
     ...
     for(i = 6; loadAddr != 04537; begovrtbl += 6, i += 6) {
       ...
       ovrsizeW <<= 1;      // in bytes
            ulong ovrstart = (inf.maxEA & ~0xF) + (loadAddr & 0xF) + 0x10;
       ulong sel_l = ushort((ovrstart >> 4) - (loadAddr >> 4));
       ...
       s->ovrname = ((ulong)numOvr << 16) | numSeg;
