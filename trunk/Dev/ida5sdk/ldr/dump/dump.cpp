...
//--------------------------------------------------------------------------
inline ulong hex(char *&word)
{
  return strtoul(word, &word, 16);
}

//--------------------------------------------------------------------------
inline ulong oct(char *&word)
{
...
//12 23 34 56 78
  ...
  int p0len = -1;        // length of the first word's hex part
...
//--------------------------------------------------------------------------
void load_file(linput_t *li, ushort _neflag, const char * /*fileformatname*/)
{
  char line[MAXSTR];
  char *words[MAXSTR];
  ...
  while ( qlgets(line, sizeof(line), li) )
  {
    ...
    // the first word must be a number (more than one digit)
    char *ptr = words[0];
    ulong w0 = octpref ? oct(ptr) : hex(ptr);
    p0len = ptr - words[0];
    // take the separator from the first line
    if ( nontrivial_line_count == 1 )
      qstrncpy(w0sep, ptr, sizeof(w0sep));
...
//        msg("ptr=%s octnum=%d\n", ptr, octnum);
      ...
      ulong b = octnum ? oct(ptr) : hex(ptr);
      int nc = ptr - words[idx];
      if ( nc < 2 )
      {
