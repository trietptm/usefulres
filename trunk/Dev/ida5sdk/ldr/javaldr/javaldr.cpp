...
//      otherwise return 0
//
static int accept_file(linput_t *li,
                       char fileformatname[MAX_FILE_FORMAT_NAME], int n)
{
  ...
  if(   n
     ...
     || lread2bytes(li, &maj_ver, 1)) goto badfmt;

  if(maj_ver <= JDK_MIN_MAJOR) {
    if(maj_ver < JDK_MIN_MAJOR) goto badfmt;
    jdk = (uchar)(maj_ver >= JDK_1_1_MINOR);
  } else if(maj_ver > JDK_MAX_MAJOR) {
badfmt:
    return(0);
  } else jdk = (uchar)(maj_ver - (JDK_MIN_MAJOR-1));

  qsnprintf(fileformatname, MAX_FILE_FORMAT_NAME,
            "JavaVM Class File (JDK 1.%u)", jdk);
  return(f_LOADER);
}
...
//
static void load_file(linput_t *li, ushort neflag,
                      const char * /*fileformatname*/)
{
  if(ph.id != PLFM_JAVA) init_loader_options(li);
