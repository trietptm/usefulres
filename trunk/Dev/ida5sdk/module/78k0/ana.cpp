...
inline ulong GetFullAddress(uchar addr)
{
...
inline void Operand_Addr16( op_t &x, ulong Res, char Target_data_type )
{
   x.type = o_mem;
   x.value = Res;
   x.addr = Res;
...
uchar code = ua_next_byte();
	...
	case 0x7C:
			...
			cmd.Op1.type = o_near;
			cmd.Op1.addr = ((long)( code & 0xF0 )<<8) | (ulong)ua_next_byte() + 0x800;
