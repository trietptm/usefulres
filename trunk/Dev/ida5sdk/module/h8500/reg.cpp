...
//------------------------------------------------------------------
static void func_header(func_t *pfn)
{
...
//    warning("Can't open %s, symbol definitions are not loaded", file);
  ...
  while ( qfgets(line, sizeof(line), fp) )
  {
    ...
    if ( i != numsyms ) break;
    syms = (sym_t *)qrealloc(syms, (numsyms+1)*sizeof(sym_t));
...
//-----------------------------------------------------------------------
static char *shnames[] = { "h8500", NULL };
static char *lnames[] = {
  "Hitachi H8/500",
  NULL
};
...
static ea_t h8_extract_address(ea_t screen_ea, const char *string, int x)
{
  size_t len = strlen(string);
  if ( len == 0 || x > len ) return BADADDR;
  if ( x == len ) x--;
  const char *ptr = string + x;
  while ( ptr > string && isxdigit(ptr[-1]) ) ptr--;
  const char *start = ptr;
  while ( isxdigit(ptr[0]) ) ptr++;
  len = ptr - start;
  char buf[MAXSTR];
