...
{
  op.dtyp = dtyp;
  if (! (byte & 0x80)) // Direct addressing mode
  {
    op.addr = byte & 0x7F;
    if (!mmr)
    {
...
static void op_MMRxy(op_t &op, int byte)
{
  byte &= 0x0F;
  if (byte <= 7) op_reg_word(op.n+1, AR0+byte);
  else op_reg_word(op.n+1, SP);
}
...
static void op_XYmem(op_t &op, int four)
{
  ...
  four = (four>>2) & 0x3;
  if (four < 3) op.IndirectAddressingMOD = four;
...
static void op_cond8(op_t &op, int byte)
{
...
static void op_cond8(int op, int byte)
{
  if (get_cond8(byte) == NULL)
  {
    cmd.itype = TMS320C54_null;
  ...
  if (op <= 3) op_cond8(cmd.Operands[op-1], byte);
  else
  {
...
static void op_cond4(op_t &op, int quart)
{
  quart &= 0xF;
  // map cond4 to cond8
...
static void ops_conds(int op, int byte)
{
  byte &= 0xFF;
  int bits7_6 = (byte >> 6) & 0x3;

  int n = 0; // number of operands added
  ...
  {
    if (n == 0) op_cond8(op+n++, COND8_UNC);
    // modify cmd.itype
    if (op-1+n <= 3) cmd.itype += n-1;
    else cmd.itype += 3-op;
  }
}
...
{
  int code = get_full_byte(cmd.ea);
  cmd.size = 1;

  int bits7_0   = code & 0xFF;
  int bits4_0   = code & 0x1F;
  int bits3_0   = code & 0xF;
  int bits2_0   = code & 0x7;
  int bits1_0   = code & 0x3;
  int bit0      = code & 1;
  int bits8_3   = (code >> 3) & 0x3F;
  int bits15_4  = code >> 4;
  int bits7_4   = bits15_4 & 0xF;
  int bits7_5   = bits7_4 >> 1;
  int bits6_5   = bits7_5 & 0x3;
  int bit7      = bits7_0 >> 7;
  int bits15_8  = code >> 8;
  int bits11_8  = bits15_8 & 0xF;
  int bits9_8   = bits11_8 & 0x3;
  int bit8      = bits9_8 & 1;
  int bits15_9  = code >> 9;
  int bits11_9  = bits15_9 & 0x7;
  int bits10_9  = bits11_9 & 0x3;
  int bit9      = bits10_9 & 1;
  int bits15_10 = code >> 10;
  int bits12_10 = bits15_10 & 0x7;
  int bits11_10 = bits12_10 & 0x3;
  int bits15_12 = code >> 12;
...
// 0011 01R1 IAAA AAAA                      MACA[R] Smem [,B]
  {
    ...
    if (cmd.itype != TMS320C54_null) op_Smem(cmd.Op1, code);
  }
...
// 0100 010D IAAA AAAA                      LD Smem, 16, dst
    ...
    if (cmd.itype != TMS320C54_null)
    {
      ...
      {
        op_src_dst(3, bit9);
        if (bit8 != bit9) op_src_dst(4, bit8);
      }
      else
        op_src_dst(3, bit8);
    }
...
// 0100 1010 IAAA AAAA                      PSHM MMR
  {
    ...
    op_MMR(cmd.Op1, code);
    if (cmd.itype == TMS320C54_ldm) op_src_dst(2, bit8);
  }
...
// 0101 111D IAAA AAAA                      DSADT Lmem, dst
  {
    ...
    if (cmd.itype != TMS320C54_dadd2) op_src_dst(2, bit8);
    else
    {
      op_src_dst(2, bit9);
      if (bit8 != bit9)
...
// 0110 01SD IAAA AAAA 16-bit constant      MAC Smem, #lk, src [,dst]
  {
    ...
    if (cmd.itype == TMS320C54_mpy3) op_src_dst(3, bit8);
...
// 0110 1111 IAAA AAAA 0000 11SD 001S HIFT  SUB Smem [,SHIFT], src [,dst]
  {
    ...
    if (cmd.itype != TMS320C54_null)
    {
      ...
      int shift = get_signed(code2, 0x1F);
      if (shift != 0)
      {
...
// 1001 0110 XXXX BITC                      BIT Xmem, BITC
  {
    ...
    if (cmd.itype != TMS320C54_bit) op_src_dst(3, bit8);
  }
...
// 1110 0011 XXXX YYYY                      ABDST Xmem, Ymem
  {
    ...
    op_Xmem(cmd.Op1, code);
    op_Ymem(cmd.Op2, code);
    if (cmd.itype == TMS320C54_firs) op_pmad_data(cmd.Op3);
  }
...
// 1110 1101 000K KKKK                      LD #k5, ASM
  {
    cmd.itype = TMS320C54_ld2;
    op_bit(cmd.Op1, get_signed(code, 0x1F), false);
    op_reg_word(2, ASM);
  }
...
// 1111 00SD 0101 SHFT 16-bit constant      XOR #lk [,SHFT], src [,dst]
    {
      ...
      if (cmd.itype != TMS320C54_null)
      {
        ...
        {
          op_src_dst(2, bit9);
          if (bit8 != bit9) op_src_dst(3, bit8);
          else cmd.itype--;
        }
      }
    }
...
// 1111 00SD 111S HIFT                      SFTL src, SHIFT [,dst]
    {
      ...
      op_src_dst(1, bit9);
      int shift = get_signed(code, 0x1F);
      int n = 2; // current operand
      if (cmd.itype == TMS320C54_sftl2 || shift != 0)
      {
        op_bit(cmd.Operands[n-1], shift);
        n++;
        if (cmd.itype != TMS320C54_sftl2) cmd.itype++;
      }
      if (bit8 != bit9)
      {
        op_src_dst(n, bit8);
...
// 1111 01SD 011S HIFT                      SFTA src, SHIFT [,dst]
    {
      ...
      if (cmd.itype == TMS320C54_sfta2 || shift != 0)
      {
        op_bit(cmd.Operands[n-1], shift);
        n++;
        if (cmd.itype != TMS320C54_sfta2) cmd.itype++;
      }
...
// 1111 01Z0 1110 1011                      RETE[D]
    {
      ...
      if(cmd.itype != TMS320C54_null)
      {
        if (cmd.itype == TMS320C54_bacc || cmd.itype == TMS320C54_cala
          ...
          op_src_dst(1, bit8);
        else if (bit8) cmd.itype = TMS320C54_null;
        if(cmd.itype != TMS320C54_null && bit9) cmd.itype++;
      }
    }
    else
      cmd.itype = TMS320C54_null;
