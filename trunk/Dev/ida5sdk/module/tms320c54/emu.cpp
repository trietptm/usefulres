...
static void process_operand(op_t &x, int use)
{
  switch ( x.type )
  {
    ...
    case o_local: // local variables
      if ( may_create_stkvars() && (get_func(cmd.ea) != NULL) && ua_stkvar(x, x.addr) )
    ...
    if ( pfn->frame == BADNODE )
    {
      ...
      while (ea < pfn->endEA) // check for register pushs
      {
        ...
        if (cmd.itype == TMS320C54_pshm
          && ( cmd.Op1.type == o_mem || cmd.Op1.type == o_mmr)
          && get_mapped_register(cmd.Op1.addr) != -1)
            regsize++;
        else break;
      }
      int localsize = 0;
...
//----------------------------------------------------------------------
int tms_get_frame_retsize(func_t * /*pfn*/)
{
