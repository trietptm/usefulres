...
static void out_cond8(char value)
{
  ...
  if (cond != NULL) out_line(cond, COLOR_REG);
  else warning("interr: out 8-bit condition");
}
    ...
    case o_imm:
      {
        ...
        if (idpflags & TMS320C54_IO && x.IOimm) name = find_sym(x.value);
        if ( !x.NoCardinal ) out_symbol('#');
        if ( name != NULL ) out_line(name, COLOR_IMPNAME);
        else
        {
          if (! x.Signed)
            OutValue(x, OOFW_IMM);
