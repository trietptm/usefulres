...
class bytes_c
{
  ...
  public:
    void reset(void) { i = 0; };
    void reload(void) { bytes[0] = get_byte(cmd.ea); size = 1; reset(); };
    bytes_c(void) {  reload(); };
    int get_next(void);
    int get(int j);
};
...
int bytes_c::get_next(void)
{
  if (i == size)
  {
    ...
    bytes[size] = get_byte(cmd.ea+size);
    size++;
  }
  return bytes[i++];
}

// get a byte already in the cache
int bytes_c::get(int j)
{
...
typedef void (*func_op_mask_t)(const mask_t *, int offset, op_t &op);

typedef struct
{
  func_op_mask_t func; // function to set operand
  int mask;            // mask of operand
} op_mask_t;

struct mask_t
{
  int code;     // full opcode of instruction
  int mask;     // full mask of instruction
  char size;    // number of bytes for instruction
  ushort itype; // itype of instruction
  op_mask_t op_mask[OP_MASK_N]; // function and mask to set operands
};


// verify if a given byte match a byte (0 <= n <= mask_t.size-1) from a code and mask
static bool byte_match_code_mask(const mask_t *mask, int byte, char n, char bytesize = 8)
{
  // compute nshift and nmask
  int nshift = (mask->size - 1 - n) * bytesize;
  int nmask  = 0;
  for (int i = 0; i < bytesize; i++) nmask = (nmask << 1) | 1;

  // shift code and mask to get current byte
  int mask_code = (mask->code >> nshift) & nmask;
  int mask_mask = (mask->mask >> nshift) & nmask;
  byte &= nmask;
...
#define OP_MASK (~(0xFFFFFFFF << OP_BITS)) // effective mask
 #define OP_OP_NULL 0
 #define OP_OP_IMM  1
...
static int get_masked_operand(unsigned int code, unsigned int mask)
{
  if (mask == 0) return 0;
  bool no = false;
  // special operations
  switch (mask >> OP_BITS)
  {
    ...
    case OP_OP_NOT:
      ...
      break;
    default:
      error("interr: ana: get_masked_operand()");
  }
  code &= (mask & OP_MASK);
  while ((mask & 1) == 0)
  {
...
// process mask->op_mask[op_mask_n] on cmd.Operands[op_n] (if op.n not modified) and eventually modify op_n
static bool process_masks_operand(const mask_t *mask, int code, int op_mask_n, int &op_n)
{
  // initialize an operand to work on
  op_t work;
  if (op_n < UA_MAXOP) work = cmd.Operands[op_n];
  else work = cmd.Operands[UA_MAXOP-1];

  const op_mask_t *op_mask = &mask->op_mask[op_mask_n];
  if (!op_mask->func) return false;
  op_mask->func(mask, get_masked_operand(code, op_mask->mask), work); // execute the operand function
  if (work.type != o_void) // if the operand was modified
  {
    if (work.n == op_n) // the function worked on the current operand
    {
      if (op_n >= UA_MAXOP) return false; // error if not enough operands in cmd
      cmd.Operands[op_n] = work; // save the new operand
      op_n++; // go to the next operand
    }
    else // the function modified a precedent operand
    {
      if (work.n >= UA_MAXOP) return false;
      cmd.Operands[work.n] = work; // save the new precedent operand (modified)
    }
  }
  return true;
}
...
static void process_masks(const mask_t *masks, ushort itype_null, char bytesize = 8)
{
  ...
  while (p->mask != 0)
  {
    bytes.reset();
    ushort nbytes = 0;
    while (nbytes < p->size && byte_match_code_mask(p, bytes.get_next(), nbytes, bytesize))
      nbytes++;
    if (nbytes == p->size) // if bytes matches all bytes from code and mask
    {
      ...
      // compute full code
      int code = 0;
      for (i = 0; i < nbytes; i++) code = (code << bytesize) | bytes.get(i);
      // process operands
      int op_n = 0;
      for (i = 0; i < OP_MASK_N; i++)
      {
        if (!process_masks_operand(p, code, i, op_n)) break;
      }
      return;
    }
    p++;
...
// permit a mask_func_n to work on the precedent operand
bool process_mask_precedent_op(op_t &op)
{
  int n;
  for (n = 0; n < 6; n++) // find the first o_void operand
    if (cmd.Operands[n].type == o_void) break;

  if (n == 0) return false; // no precedent operand
  op = cmd.Operands[n-1]; // copy the precedent operand to the current position
  return true;
}
...
// find instruction cmd+"chars" with same operands:
...
//  cmd1 - cmd2 - ... - cmd"chars"1 - cmd"chars"2 - ...
static bool find_cmd_suffix(const char *chars, ushort itype_last, bool parallel, bool name1_chars, bool name2_chars)
{
  const char *cmd_name1 = Instructions[cmd.itype].name;
  char cmd_name1_chars[MAXSTR];
  qsnprintf(cmd_name1_chars, sizeof(cmd_name1_chars),
                        name1_chars ? "%s%s" : "%s", cmd_name1, chars);

  const char *cmd_name2;
  char cmd_name2_chars[MAXSTR];
  if (parallel)
  {
    cmd_name2 = strchr(cmd_name1, 0)+1;
    qsnprintf(cmd_name2_chars, sizeof(cmd_name2_chars),
                        name2_chars ? "%s%s" : "%s", cmd_name2, chars);
  }

  ushort i = cmd.itype + 1;
  // jump over same instruction names (but eventually different number of params)
  while (!strcmp(Instructions[i].name, cmd_name1)
    && (!parallel || !strcmp(strchr(Instructions[i].name,0)+1, cmd_name2)))
      if (++i == itype_last) return false;

  // loop until current instruction names are the same as what we are searching for (name or name+chars)
  while (strcmp(Instructions[i].name, cmd_name1_chars)
    || ( parallel && strcmp(strchr(Instructions[i].name,0)+1, cmd_name2_chars)))
      if (++i == itype_last) return false;

 // loop until current instruction has same params
  ulong cmd_feature = Instructions[cmd.itype].feature;
  while (Instructions[i].feature != cmd_feature)
    if (++i == itype_last) return false;
...
#define cmd_chars(NAME,CHARS,PARALLEL,NAME1_CHARS,NAME2_CHARS)                    \
  static void NAME(const mask_t *, int offset, op_t &)                            \
  {                                                                               \
    if ((offset & 1)                                                              \
      && !find_cmd_suffix(CHARS,TMS320C55_last,PARALLEL,NAME1_CHARS,NAME2_CHARS)) \
        error("interr: ana: adjust_cmd_suffix");                                  \
  }
cmd_chars(cmd_1_R_2_R,"r",true,true,true)    // cmd_1_R_2_R   %
cmd_chars(cmd_1_40_2_40,"40",true,true,true) // cmd_1_40_2_40 g
cmd_chars(cmd_1_2_R,"r",true,false,true)     // cmd_1_2_R     %
cmd_chars(cmd_1_2_40,"40",true,false,true)   // cmd_1_2_40    g
cmd_chars(cmd_1_R,"r",false,true,false)      // cmd_1_R       %
cmd_chars(cmd_1_40,"40",false,true,false)    // cmd_1_40      g
cmd_chars(cmd_1_R_2,"r",true,true,false)     // cmd_1_R_2     %
cmd_chars(cmd_1_U,"u",false,true,false)      // cmd_1_U       u
cmd_chars(cmd_1_P,"p",false,true,false)      // cmd_1_P       swap()
cmd_chars(cmd_1_4,"4",false,true,false)      // cmd_1_4       swap()

// set user parellel
static void cmd_UP(const mask_t *, int, op_t &)
{
  cmd.SpecialModes |= TMS_MODE_USER_PARALLEL;
}

// built-in parallelism
static void blt_prll(const mask_t *, int, op_t &) // count the number of actual operands
{
  cmd.Parallel = 0;
  for (int i = 0; i < UA_MAXOP; i++)
  {
    if (cmd.Operands[i].type == o_void) break;
    cmd.Parallel++;
  }
}

// simulated user parallelism
//static void usr_prll(const mask_t *mask, int offset, op_t &op)
//{
//  if (offset & 1)
//  {
//    blt_prll(mask, offset, op);
//    cmd.SpecialModes |= TMS_MODE_SIMULATE_USER_PARALLEL;
//  }
//}
...
#define op_imm(NAME,DTYP)                                     \
  static void op_##NAME(const mask_t *, int offset, op_t &op) \
  {                                                           \
    op.type  = o_imm;                                         \
    op.value = offset;                                        \
    op.dtyp  = dt_##DTYP;                                     \
...
static void op_min_k4(const mask_t *, int offset, op_t &op)
{
  op.type  = o_imm;
  op.value = -offset;
  op.dtyp  = dt_byte;
...
static void op_K8(const mask_t *, int offset, op_t &op)
{
  op.type  = o_imm;
  op.value = get_signed(offset, 0xFF);
  op.dtyp  = dt_byte;
  op.tms_signed = true;
}

static void op_K16(const mask_t *, int offset, op_t &op)
{
  op.type  = o_imm;
  op.value = get_signed(offset, 0xFFFF);
  op.dtyp  = dt_word;
  op.tms_signed = true;
}

static void op_1(const mask_t *, int, op_t &op)
{
  op.type  = o_imm;
  op.value = 1;
  op.dtyp  = dt_byte;
...
static void op_min_1(const mask_t *, int, op_t &op)
{
  op.type  = o_imm;
  op.value = uval_t(-1);
  op.dtyp  = dt_byte;
...
char optional_op; // indice of an optional operand

static void op_src(const mask_t *, int offset, op_t &op) // FSSS, FDDD
{
  op.type = o_reg;
  op.dtyp = dt_word;
  op.reg  = AC0 + offset;
...
// optional operand
static void opt_src(const mask_t *mask, int offset, op_t &op) // FSSS, FDDD
{
  if (optional_op != -1 && cmd.Operands[optional_op].type == o_reg
    && cmd.Operands[optional_op].reg == AC0 + offset)
      return; // no operand if same than the source
  // add the operand
  op_src(mask, offset, op);
  optional_op = -1;
  cmd.itype++;
}
#define opt_dst opt_src
#define opt_ACx opt_src
...
static void op_xsrc(const mask_t *, int offset, op_t &op) // XSSS, XDDD
...
static void op_Xmem(const mask_t *mask, int offset, op_t &op) // XXXMMM, YYYMMM
{
  op_ARx(mask, offset >> 3, op);
  op.tms_modifier = TMS_MODIFIER_REG + (offset & 0x7);
}
#define op_Ymem op_Xmem

static void op_Cmem(const mask_t *mask, int offset, op_t &op) // mm
{
  op_CDP(mask, 0, op);
  op.tms_modifier = TMS_MODIFIER_REG + (offset & 0x3);
}

static void op_mem(const mask_t *mask, int offset, op_t &op, char dtyp, bool ARn_mod = false) // AAAAAAAI
{
  cmd.OpMem = 1 + op.n;
  if (!(offset & 1)) // @dma
    ...
    if ((offset & 0x1F) == 0x11) // xxx1 0001
    {
      ...
      switch(bits)
      {
        case 0: // *ABS16(#k16)
          op.type         = o_mem;
          op.tms_regH     = DPH;
          op.addr         = (get_byte(cmd.ea+cmd.size) << 8) | get_byte(cmd.ea+cmd.size+1);
          op.tms_modifier = TMS_MODIFIER_ABS16;
          cmd.size += 2;
          break;
        case 1: // *(#k23)
          op.type         = o_mem;
          op.addr         = (get_byte(cmd.ea+cmd.size) << 16) | (get_byte(cmd.ea+cmd.size+1) << 8) | get_byte(cmd.ea+cmd.size+2);
        ...
        case 5: // *CDP-
          op_CDP(mask, 0, op);
          op.tms_modifier= TMS_MODIFIER_REG + bits-3;
          break;
        case 6: // *CDP(#K16)
        case 7: // *+CDP(#K16)
          op_CDP(mask, 0, op);
          op.value = get_signed( (get_byte(cmd.ea+cmd.size) << 8) | get_byte(cmd.ea+cmd.size+1), 0xFFFF );
          op.tms_modifier = TMS_MODIFIER_REG_OFFSET + bits-6;
          cmd.size += 2;
          break;
      }
    }
    else
    {
      ...
      if ((offset & 0x11) == 0x01) // xxx0 xxx1
      {
        switch(bits)
        {
          ...
          case 1: // *ARn+
          case 2: // *ARn-
            op.tms_modifier= TMS_MODIFIER_REG + bits;
            break;
          case 3: // *(ARn+T0)
      ...
      {
        int bits = (offset >> 1) & 0x7;
        sel_t arms = getSR(cmd.ea, ARMS);
        if ( arms == BADSEL ) arms = 0;
        if (ARn_mod || !arms)
        {
          switch(bits)
          {
            ...
            case 6: // *(ARn+T0B)
            case 7: // *(ARn-T0B)
              op.tms_modifier= TMS_MODIFIER_REG_T1 + bits-3;
              break;
          }
        }
        else
...
static void op_Smem(const mask_t *mask, int offset, op_t &op) // AAAAAAAI
{
  op_mem(mask, offset, op, dt_word);
}

static void op_Lmem(const mask_t *mask, int offset, op_t &op) // AAAAAAAI
{
  op_mem(mask, offset, op, dt_dword);
}

static void op_ARn_mod(const mask_t *mask, int offset, op_t &op) // AAAAAAAI
{
  op_mem(mask, offset, op, dt_word, true);
}

// @dma = bit number
static void op_Baddr(const mask_t *mask, int offset, op_t &op) // AAAAAAAI
{
  if (!(offset & 1)) // @dma
  {
    op.type  = o_imm;
  ...
  else op_mem(mask, offset, op, dt_byte);
}

#define fn_operator(NAME,OPERATOR)                            \
  static void fn_##NAME(const mask_t *, int offset, op_t &op) \
  {                                                           \
    if (offset & 1)                                           \
    {                                                         \
      process_mask_precedent_op(op);                          \
...
static void op_BitIn(const mask_t *, int offset, op_t &op)
{
  op.type = o_reg;
  op.dtyp = dt_byte;
...
static void op_swap(const mask_t *mask, int offset, op_t &op)
{
  cmd.Op1.type = o_reg;
  ...
  switch (offset)
  {
    case 0x00: cmd.Op1.reg = AC0; cmd.Op2.reg = AC2; break;
    case 0x01: cmd.Op1.reg = AC1; cmd.Op2.reg = AC3; break;
    case 0x04: cmd.Op1.reg = T0;  cmd.Op2.reg = T2;  break;
    case 0x05: cmd.Op1.reg = T1;  cmd.Op2.reg = T3;  break;
    case 0x08: cmd.Op1.reg = AR0; cmd.Op2.reg = AR2; break;
    case 0x09: cmd.Op1.reg = AR1; cmd.Op2.reg = AR3; break;
    case 0x0C: cmd.Op1.reg = AR4; cmd.Op2.reg = T0;  break;
    case 0x0D: cmd.Op1.reg = AR5; cmd.Op2.reg = T1;  break;
    case 0x0E: cmd.Op1.reg = AR6; cmd.Op2.reg = T2;  break;
    case 0x0F: cmd.Op1.reg = AR7; cmd.Op2.reg = T3;  break;
    case 0x10: cmd_1_P(mask, 1, op); cmd.Op1.reg = AC0; cmd.Op2.reg = AC2; break;
    case 0x14: cmd_1_P(mask, 1, op); cmd.Op1.reg = T0;  cmd.Op2.reg = T2;  break;
    case 0x18: cmd_1_P(mask, 1, op); cmd.Op1.reg = AR0; cmd.Op2.reg = AR2; break;
    case 0x1C: cmd_1_P(mask, 1, op); cmd.Op1.reg = AR4; cmd.Op2.reg = T0;  break;
    case 0x1E: cmd_1_P(mask, 1, op); cmd.Op1.reg = AR6; cmd.Op2.reg = T2;  break;
    case 0x2C: cmd_1_4(mask, 1, op); cmd.Op1.reg = AR4; cmd.Op2.reg = T0;  break;
    case 0x38: cmd.Op1.reg = AR0; cmd.Op2.reg = AR1; break;
    default:   cmd.itype = TMS320C55_null;
  }
}
...
static void op_l4(const mask_t *, int offset, op_t &op) // llll
{
  op.type = o_near;
  op.addr = (cmd.ip + cmd.size + offset) & 0xFFFFFF;
  op.dtyp = dt_code;
}

static void op_L7(const mask_t *, int offset, op_t &op) // LLLLLLL
{
  op.type = o_near;
  op.addr = (cmd.ip + cmd.size + get_signed(offset, 0x7F)) & 0xFFFFFF;
  op.dtyp = dt_code;
}

static void op_L8(const mask_t *, int offset, op_t &op) // LLLLLLLL
{
  op.type = o_near;
  op.addr = (cmd.ip + cmd.size + get_signed(offset, 0xFF)) & 0xFFFFFF;
  op.dtyp = dt_code;
}

static void op_L16(const mask_t *, int offset, op_t &op) // LLLLLLLL LLLLLLLL
{
  op.type = o_near;
  op.addr = (cmd.ip + cmd.size + get_signed(offset, 0xFFFF)) & 0xFFFFFF;
  op.dtyp = dt_code;
}

static void op_pmad(const mask_t *, int offset, op_t &op)
{
  op.type = o_near;
  op.addr = (cmd.ip + cmd.size + offset) & 0xFFFFFF;
  op.dtyp = dt_code;
...
static void op_P24(const mask_t *, int, op_t &op) // get_byte included
{
  op.type = o_near;
  op.addr = (get_byte(cmd.ea+cmd.size) << 16) | (get_byte(cmd.ea+cmd.size+1) << 8) | get_byte(cmd.ea+cmd.size+2);
  op.dtyp = dt_code;
...
static void op_D16(const mask_t *, int offset, op_t &op)
{
  op.type = o_mem;
  op.addr = offset;
  op.dtyp = dt_word;
...
static void op_SHIFTW(const mask_t *, int offset, op_t &op)
{
  op.type  = o_shift;
  op.value = (signed short)get_signed(offset, 0x3F);
}

// shift left
static void shl_SHIFTW(const mask_t *, int offset, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTL_IMM;
  op.tms_shift_value = (signed short)get_signed(offset, 0x3F);
}

// shift left out of brackets
static void slo_SHIFTW(const mask_t *mask, int offset, op_t &op)
{
  shl_SHIFTW(mask, offset, op);
  op.tms_shift |= TMS_OP_SHIFT_OUT;
}

// shift right
static void shl_SHFT(const mask_t *, int offset, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTL_IMM;
  op.tms_shift_value = offset;
}

static void shl_Tx(const mask_t *, int offset, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTL_REG;
  op.tms_shift_value = T0 + offset;
}

static void shl_T2(const mask_t *, int, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTL_REG;
...
static void shl_16(const mask_t *, int, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTL_IMM;
...
static void shr(const mask_t *, int imm, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTR_IMM;
  op.tms_shift_value = imm;
}

static void eq_K16(const mask_t *, int offset, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_EQ;
  op.tms_shift_value = get_signed(offset, 0xFFFF);
}

static void neq_0(const mask_t *, int, op_t &op)
{
  process_mask_precedent_op(op);
...
static void RELOP(const mask_t *, int offset, op_t &op) // cc
{
  process_mask_precedent_op(op);
  op.type      = o_relop;
...
static void RELOP_dst(const mask_t *mask, int offset, op_t &op)
{
  process_mask_precedent_op(op);
...
static void RELOP_K8(const mask_t *, int offset, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_relop_type = TMS_RELOP_IMM;
  op.value = (signed short)get_signed(offset, 0xFF);
}


static void op_cond(const mask_t *mask, int offset, op_t &op) // CCC CCCC
{
  op.reg = 0;
  if ((offset>>4) <= 0x5) // <= 101 xxxx
  { // src <comparison operator> 0
    op_src(mask, offset & 0xF, op);
    op.type  = o_cond;
    op.value = offset & 0x70;
  }
  else if ((offset>>2) == 0x18) // == 110 00xx
  { // overflow(ACx)
    op_src(mask, offset & 0x3, op);
    op.type  = o_cond;
  ...
  { // TC1, TC2, CARRY, reserved
    if (offset == 0x67) // 110 0111
      cmd.itype = TMS320C55_null; // reserved
    else
    {
      op.type  = o_cond;
      op.value = offset;
...
static mask_t masks[] =
{
  ...
  { 0x120000,   0xFE0300,   3, TMS320C55_cmp,       {{ cmd_1_U,     0x000004         },{ op_src,        0x00F000   },{ RELOP,      0x000C00         },{ RELOP_dst, 0x0000F0   },{ op_TCx,     0x000001   }}},
  { 0x120100,   0xFE0308,   3, TMS320C55_cmpand,    {{ cmd_1_U,     0x000004         },{ op_src,        0x00F000   },{ RELOP,      0x000C00         },{ RELOP_dst, 0x0000F0   },{ op_TCy,     0x000002   },{ op_TCx,    0x000001   }}},
  { 0x120108,   0xFE0308,   3, TMS320C55_cmpand,    {{ cmd_1_U,     0x000004         },{ op_src,        0x00F000   },{ RELOP,      0x000C00         },{ RELOP_dst, 0x0000F0   },{ op_TCy,     0x000002   },{ fn_not,    OP_TRUE    },{ op_TCx,    0x000001   }}},
  { 0x120200,   0xFE0308,   3, TMS320C55_cmpor,     {{ cmd_1_U,     OP_NOT(0x000004) },{ op_src,        0x00F000   },{ RELOP,      0x000C00         },{ RELOP_dst, 0x0000F0   },{ op_TCy,     0x000002   },{ op_TCx,    0x000001   }}},
  { 0x120208,   0xFE0308,   3, TMS320C55_cmpor,     {{ cmd_1_U,     OP_NOT(0x000004) },{ op_src,        0x00F000   },{ RELOP,      0x000C00         },{ RELOP_dst, 0x0000F0   },{ op_TCy,     0x000002   },{ fn_not,    OP_TRUE    },{ op_TCx,    0x000001   }}},
  ...
  { 0x1E0000,   0xFE0002,   3, TMS320C55_mpyk2,     {{ cmd_1_R,     0x000001         },{ op_K8,         0x00FF00   },{ op_ACx,     0x0000C0         },{ opt_ACy,   0x000030   }}},
  { 0x1E0002,   0xFE0002,   3, TMS320C55_mack3,     {{ cmd_1_R,     0x000001         },{ op_Tx,         0x00000C   },{ op_K8,      0x00FF00         },{ op_ACx,    0x0000C0   },{ opt_ACy,    0x000030   }}},
  ...
  { 0x5402,     0xFE0E,     2, TMS320C55_sqa1,      {{ cmd_1_R,     0x0001           },{ op_ACx,        0x0030     },{ opt_ACy,    0x00C0           }}},
  { 0x5404,     0xFE0E,     2, TMS320C55_sqs1,      {{ cmd_1_R,     0x0001           },{ op_ACx,        0x0030     },{ opt_ACy,    0x00C0           }}},
  { 0x5406,     0xFE0E,     2, TMS320C55_mpy1,      {{ cmd_1_R,     0x0001           },{ op_ACx,        0x0030     },{ opt_ACy,    0x00C0           }}},
  { 0x5408,     0xFE0E,     2, TMS320C55_sqr1,      {{ cmd_1_R,     0x0001           },{ op_ACx,        0x0030     },{ opt_ACy,    0x00C0           }}},
  { 0x540A,     0xFE0E,     2, TMS320C55_round1,    {{ op_ACx,      0x0030           },{ opt_ACy,       0x00C0     }}}, // error manual: unused bits
  { 0x540C,     0xFE0E,     2, TMS320C55_sat1,      {{ cmd_1_R,     0x0001           },{ op_ACx,        0x0030     },{ opt_ACy,    0x00C0           }}},
  { 0x5600,     0xFE02,     2, TMS320C55_mac3,      {{ cmd_1_R,     0x0001           },{ op_ACx,        0x0030     },{ op_Tx,      0x000C           },{ opt_ACy,   0x00C0     },{ op_ACy,     0x00C0     }}},
  { 0x5602,     0xFE02,     2, TMS320C55_mas2,      {{ cmd_1_R,     0x0001           },{ op_Tx,         0x000C     },{ op_ACx,     0x0030           },{ opt_ACy,   0x00C0     }}},
  { 0x5800,     0xFE02,     2, TMS320C55_mpy2,      {{ cmd_1_R,     0x0001           },{ op_Tx,         0x000C     },{ op_ACx,     0x0030           },{ opt_ACy,   0x00C0     }}},
  { 0x5802,     0xFE02,     2, TMS320C55_mac4,      {{ cmd_1_R,     0x0001           },{ op_ACy,        0x00C0     },{ op_Tx,      0x000C           },{ op_ACx,    0x0030     },{ op_ACy,     0x00C0     }}},
  ...
  { 0x6F000000, 0xFF000000, 4, TMS320C55_bcc,       {{ cmd_1_U,     0x00010000       },{ op_L8,         0x000000FF },{ op_src,     0x00F00000       },{ RELOP,     0x000C0000 },{ RELOP_K8,   0x0000FF00 }}},
  ...
  { 0x79000000, 0xFF000002, 4, TMS320C55_mpyk2,     {{ cmd_1_R,     0x00000001       },{ op_K16,        0x00FFFF00 },{ op_ACx,     0x000000C0       },{ opt_ACy,   0x00000030 }}},
  { 0x79000002, 0xFF000002, 4, TMS320C55_mack3,     {{ cmd_1_R,     0x00000001       },{ op_Tx,         0x0000000C },{ op_K16,     0x00FFFF00       },{ op_ACx,    0x000000C0 },{ opt_ACy,    0x00000030 }}},
  ...
  { 0x82000000, 0xFF000C00, 4, TMS320C55_mpy_mpy,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ blt_prll, 0          },{ op_Ymem,  0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns,  0x00000040 },{ op_ACy, 0x0000000C }}},
  { 0x82000400, 0xFF000C00, 4, TMS320C55_mac_mpy,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ blt_prll, 0          },{ op_Ymem,  0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns,  0x00000040 },{ op_ACy, 0x0000000C }}},
  { 0x82000800, 0xFF000C00, 4, TMS320C55_mas_mpy,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ blt_prll, 0          },{ op_Ymem,  0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns,  0x00000040 },{ op_ACy, 0x0000000C }}},
  { 0x82000C00, 0xFF000C00, 4, TMS320C55_amar_mpy,  {{ cmd_1_2_R,   0x00000001       },{ cmd_1_2_40,    0x00000002 },{ op_Xmem,    0x00FC0000       },{ blt_prll,  0,         },{ op_Ymem,    0x0003F000 },{ fn_uns,    0x00000080 },{ op_Cmem,   0x00000300 },{ fn_uns,   0x00000040 },{ op_ACx,   0x0000000C }}},
  { 0x83000000, 0xFF000C00, 4, TMS320C55_mac_mac,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ blt_prll, 0          },{ op_Ymem,  0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns,  0x00000040 },{ op_ACy, 0x0000000C }}},
  { 0x83000400, 0xFF000C00, 4, TMS320C55_mas_mac,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ blt_prll, 0          },{ op_Ymem,  0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns,  0x00000040 },{ op_ACy, 0x0000000C }}},
  { 0x83000800, 0xFF000C00, 4, TMS320C55_mac_mac,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ shr,      OP_IMM(16) },{ blt_prll, 0          },{ op_Ymem, 0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns, 0x00000040 },{ op_ACy, 0x0000000C }}},
  { 0x83000C00, 0xFF000C00, 4, TMS320C55_amar_mac,  {{ cmd_1_2_R,   0x00000001       },{ cmd_1_2_40,    0x00000002 },{ op_Xmem,    0x00FC0000       },{ blt_prll,  0,         },{ op_Ymem,    0x0003F000 },{ fn_uns,    0x00000080 },{ op_Cmem,   0x00000300 },{ fn_uns,   0x00000040 },{ op_ACx,   0x0000000C }}},
  { 0x84000000, 0xFF000C00, 4, TMS320C55_mas_mac,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ blt_prll, 0          },{ op_Ymem,  0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns,  0x00000040 },{ op_ACy, 0x0000000C },{ shr,    OP_IMM(16) }}},
  { 0x84000400, 0xFF000C00, 4, TMS320C55_amar_mac,  {{ cmd_1_2_R,   0x00000001       },{ cmd_1_2_40,    0x00000002 },{ op_Xmem,    0x00FC0000       },{ blt_prll,  0,         },{ op_Ymem,    0x0003F000 },{ fn_uns,    0x00000080 },{ op_Cmem,   0x00000300 },{ fn_uns,   0x00000040 },{ op_ACx,   0x0000000C },{ shr,     OP_IMM(16) }}},
  { 0x84000800, 0xFF000C00, 4, TMS320C55_mpy_mac,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ blt_prll, 0          },{ op_Ymem,  0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns,  0x00000040 },{ op_ACy, 0x0000000C },{ shr,    OP_IMM(16) }}},
  { 0x84000C00, 0xFF000C00, 4, TMS320C55_mac_mac,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ shr,      OP_IMM(16) },{ blt_prll, 0          },{ op_Ymem, 0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns, 0x00000040 },{ op_ACy, 0x0000000C },{ shr, OP_IMM(16) }}},
  { 0x85000000, 0xFF000C00, 4, TMS320C55_amar_mas,  {{ cmd_1_2_R,   0x00000001       },{ cmd_1_2_40,    0x00000002 },{ op_Xmem,    0x00FC0000       },{ blt_prll,  0,         },{ op_Ymem,    0x0003F000 },{ fn_uns,    0x00000080 },{ op_Cmem,   0x00000300 },{ fn_uns,   0x00000040 },{ op_ACx,   0x0000000C }}},
  { 0x85000400, 0xFF000C00, 4, TMS320C55_mas_mas,   {{ cmd_1_R_2_R, 0x00000001       },{ cmd_1_40_2_40, 0x00000002 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000080 },{ op_Cmem,    0x00000300 },{ fn_uns,    0x00000040 },{ op_ACx,    0x00000030 },{ blt_prll, 0          },{ op_Ymem,  0x0003F000 },{ fn_uns,  0x00000080 },{ op_Cmem, 0x00000300 },{ fn_uns,  0x00000040 },{ op_ACy, 0x0000000C }}},
  ...
  { 0x86000000, 0xFF0000E0, 4, TMS320C55_mpym3,     {{ cmd_1_R,     0x00000001       },{ cmd_1_40,      0x00000010 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000008 },{ fn_T3,      0x00000002 },{ op_Ymem,   0x0003F000 },{ fn_uns,    0x00000004 },{ op_ACx,   0x00000300 }}},
  { 0x86000020, 0xFF0000E0, 4, TMS320C55_macm3,     {{ cmd_1_R,     0x00000001       },{ cmd_1_40,      0x00000010 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000008 },{ fn_T3,      0x00000002 },{ op_Ymem,   0x0003F000 },{ fn_uns,    0x00000004 },{ op_ACx,   0x00000C00 },{ opt_ACy,  0x00000300 }}},
  { 0x86000040, 0xFF0000E0, 4, TMS320C55_macm3,     {{ cmd_1_R,     0x00000001       },{ cmd_1_40,      0x00000010 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000008 },{ fn_T3,      0x00000002 },{ op_Ymem,   0x0003F000 },{ fn_uns,    0x00000004 },{ op_ACx,   0x00000C00 },{ shr,      OP_IMM(16) },{ opt_ACy, 0x00000300 }}},
  { 0x86000060, 0xFF0000E0, 4, TMS320C55_masm3,     {{ cmd_1_R,     0x00000001       },{ cmd_1_40,      0x00000010 },{ op_Xmem,    0x00FC0000       },{ fn_uns,    0x00000008 },{ fn_T3,      0x00000002 },{ op_Ymem,   0x0003F000 },{ fn_uns,    0x00000004 },{ op_ACx,   0x00000C00 },{ opt_ACy,  0x00000300 }}},
  { 0x86000080, 0xFF0000E0, 4, TMS320C55_masm_mov,  {{ cmd_1_R_2,   0x00000001       },{ op_Xmem,       0x00FC0000 },{ fn_T3,      0x00000002       },{ op_Tx,     0x0000000C },{ op_ACx,     0x00000C00 },{ blt_prll,  0,         },{ op_Ymem,   0x0003F000 },{ shl_16,   0          },{ op_ACy,   0x00000300 }}},
  { 0x860000A0, 0xFF0000E0, 4, TMS320C55_macm_mov,  {{ cmd_1_R_2,   0x00000001       },{ op_Xmem,       0x00FC0000 },{ fn_T3,      0x00000002       },{ op_Tx,     0x0000000C },{ op_ACx,     0x00000C00 },{ blt_prll,  0,         },{ op_Ymem,   0x0003F000 },{ shl_16,   0          },{ op_ACy,   0x00000300 }}},
  ...
  { 0x87000000, 0xFF0000E0, 4, TMS320C55_mpym_mov,  {{ cmd_1_R_2,   0x00000001       },{ op_Xmem,       0x00FC0000 },{ fn_T3,      0x00000002       },{ op_Tx,     0x0000000C },{ op_ACy,     0x00000300 },{ blt_prll,  0,         },{ op_ACx,    0x00000C00 },{ fn_hi,    OP_TRUE    },{ shl_T2,   0          },{ op_Ymem, 0x0003F000 }}},
  { 0x87000020, 0xFF0000E0, 4, TMS320C55_macm_mov,  {{ cmd_1_R_2,   0x00000001       },{ op_Xmem,       0x00FC0000 },{ fn_T3,      0x00000002       },{ op_Tx,     0x0000000C },{ op_ACy,     0x00000300 },{ blt_prll,  0,         },{ op_ACx,    0x00000C00 },{ fn_hi,    OP_TRUE    },{ shl_T2,   0          },{ op_Ymem, 0x0003F000 }}},
  { 0x87000040, 0xFF0000E0, 4, TMS320C55_masm_mov,  {{ cmd_1_R_2,   0x00000001       },{ op_Xmem,       0x00FC0000 },{ fn_T3,      0x00000002       },{ op_Tx,     0x0000000C },{ op_ACy,     0x00000300 },{ blt_prll,  0,         },{ op_ACx,    0x00000C00 },{ fn_hi,    OP_TRUE    },{ shl_T2,   0          },{ op_Ymem, 0x0003F000 }}},
  ...
  // <instruction>.LR
  // <instruction>.CR
  { 0x9E00,     0xFF80,     2, TMS320C55_xcc,       {{ op_cond,     0x007F           },{ cmd_UP,        0          }}},
  ...
  { 0xD10000,   0xFF000C,   3, TMS320C55_mpym3,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_Cmem,   0x000003   },{ op_ACx,     0x000030   }}},
  { 0xD10004,   0xFF000C,   3, TMS320C55_macm3,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_Cmem,   0x000003   },{ op_ACx,     0x000030   }}},
  { 0xD10008,   0xFF000C,   3, TMS320C55_masm3,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_Cmem,   0x000003   },{ op_ACx,     0x000030   }}},
  { 0xD20000,   0xFF000C,   3, TMS320C55_macm2,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_ACx,    0x000003   },{ opt_ACy,    0x000030   }}},
  { 0xD20004,   0xFF000C,   3, TMS320C55_masm2,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_ACx,    0x000003   },{ opt_ACy,    0x000030   }}},
  { 0xD20008,   0xFF000C,   3, TMS320C55_sqam2,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_ACx,    0x000003   },{ opt_ACy,    0x000030   }}},
  { 0xD2000C,   0xFF000C,   3, TMS320C55_sqsm2,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_ACx,    0x000003   },{ opt_ACy,    0x000030   }}},
  { 0xD30000,   0xFF000C,   3, TMS320C55_mpym2,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_ACx,    0x000003   },{ opt_ACy,    0x000030   }}},
  { 0xD30008,   0xFF000C,   3, TMS320C55_sqrm,      {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_ACx,    0x000030   }}},
  { 0xD30004,   0xFF0004,   3, TMS320C55_mpym3,     {{ cmd_1_R,     0x000040         },{ cmd_1_U,       0x000008   },{ op_Smem,    0x00FF00         },{ fn_T3,     0x000080   },{ op_Tx,      0x000003   },{ op_ACx,    0x000030   }}},
  { 0xD40000,   0xFF0000,   3, TMS320C55_macm3,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_Tx,     0x00000C   },{ op_ACx,     0x000003   },{ opt_ACy,   0x000030   }}},
  { 0xD50000,   0xFF0000,   3, TMS320C55_masm3,     {{ cmd_1_R,     0x000040         },{ op_Smem,       0x00FF00   },{ fn_T3,      0x000080         },{ op_Tx,     0x00000C   },{ op_ACx,     0x000003   },{ opt_ACy,   0x000030   }}},
  ...
  { 0xDF0000,   0xFF000E,   3, TMS320C55_mov2,      {{ op_Smem,     0x00FF00         },{ fn_hb,         OP_TRUE    },{ fn_uns,     OP_NOT(0x000001) },{ op_dst,    0x000030   }}},
  { 0xDF0002,   0xFF000E,   3, TMS320C55_mov2,      {{ op_Smem,     0x00FF00         },{ fn_lb,         OP_TRUE    },{ fn_uns,     OP_NOT(0x000001) },{ op_dst,    0x000030   }}},
  ...
  { 0xEB0004,   0xFF000C,   3, TMS320C55_mov2,      {{ op_RETA,     0                },{ op_Lmem,       0x00FF00   },{ fn_dbl,     OP_TRUE          }}},
  ...
  { 0xED0008,   0xFF000E,   3, TMS320C55_mov2,      {{ cmd_1_40,    0x000001         },{ op_Lmem,       0x00FF00   },{ fn_dbl,     OP_TRUE          },{ op_ACx,    0x000030   }}},
  ...
  { 0xED000E,   0xFF000E,   3, TMS320C55_mov2,      {{ op_Lmem,     0x00FF00         },{ fn_dbl,        OP_TRUE    },{ op_TAx,     0x0000F0         },{ fn_pair,   OP_TRUE    }}},
  ...
  { 0xEE0004,   0xFF000E,   3, TMS320C55_sub3,      {{ op_ACx,      0x0000C0         },{ op_Lmem,       0x00FF00   },{ fn_dual,    OP_TRUE          },{ op_ACy,    0x000030   }}},
  ...
  { 0xF8000000, 0xFF000004, 4, TMS320C55_mpymk,     {{ cmd_1_R,     0x00000001       },{ op_Smem,       0x00FF0000 },{ fn_T3,      0x00000002       },{ op_K8,     0x0000FF00 },{ op_ACx,     0x00000030 }}},
  { 0xF8000004, 0xFF000004, 4, TMS320C55_macmk3,    {{ cmd_1_R,     0x00000001       },{ op_Smem,       0x00FF0000 },{ fn_T3,      0x00000002       },{ op_K8,     0x0000FF00 },{ op_ACx,     0x000000C0 },{ opt_ACy,   0x00000030 }}},
...
//--------------------------------------------------------------------------
void ana_status_bits(void)
{
  if ((cmd.itype == TMS320C55_bclr2 || cmd.itype == TMS320C55_bset2)
   && cmd.Op2.type == o_reg && cmd.Op1.type == o_imm )
  {
    int reg = -1;
    switch (cmd.Op2.reg)
    {
    ...
    if (reg != -1)
    {
      cmd.itype = (cmd.itype == TMS320C55_bclr2) ? TMS320C55_bclr1 : TMS320C55_bset1;
      cmd.Op1.type = o_reg;
      cmd.Op1.reg = reg;
      cmd.Op2.type = o_void;
    }
...
int ana(void)
{
  switch ( ptype )
  {
    case TMS320C55:
      {
        optional_op = -1;
        process_masks(masks, TMS320C55_null, 8); // analyze opcode

        // analyze special bits access
        ana_status_bits();

        // analyze user-parallelized instructions
        uchar firstbyte = get_byte(cmd.ea);
        if (firstbyte <= MAX_BYTE_USER_PARALLELIZED && (firstbyte & 1)) // instruction has E bit set
          cmd.SpecialModes |= TMS_MODE_USER_PARALLEL;

        // analyze postfixes
        uchar nextbyte = get_byte(cmd.ea+cmd.size); // isEnabled() not necessary here
        switch (nextbyte)
        {
          case BYTE_MMAP: // mmap()
            if (cmd.OpMem != 0)
            {
              int n = cmd.OpMem - 1;
              if (cmd.Operands[n].type == o_mem && cmd.Operands[n].tms_regH == DPH
                && cmd.Operands[n].tms_regP == DP && cmd.Operands[n].tms_modifier == TMS_MODIFIER_DMA)
              { // @dma using DP
                cmd.Operands[n].addr         = cmd.Operands[n].addr;
                cmd.Operands[n].type = o_mem;
                cmd.Operands[n].tms_regH     = 0;
                cmd.Operands[n].tms_regP     = 0;
                cmd.Operands[n].tms_modifier = TMS_MODIFIER_MMAP;
                cmd.size++;
              }
              else if (cmd.Operands[n].type == o_reg && cmd.Operands[n].reg == SP
                && cmd.Operands[n].tms_modifier == TMS_MODIFIER_REG_OFFSET)
              { // @dma using SP
                cmd.Operands[n].addr         = cmd.Operands[n].value;
                cmd.Operands[n].type = o_mem;
                cmd.Operands[n].tms_regH     = 0;
                cmd.Operands[n].tms_regP     = 0;
                cmd.Operands[n].tms_modifier = TMS_MODIFIER_MMAP;
          ...
          case BYTE_PORT2: // port()
            if (cmd.OpMem != 0)
            {
              ...
              {
                case o_mem:
                  cmd.Operands[n].type         = o_io;
                  cmd.Operands[n].tms_regH     = 0;
                  cmd.Operands[n].tms_regP     = PDP;
                  cmd.Operands[n].addr         = cmd.Operands[n].addr >> 1;
                  cmd.Operands[n].tms_modifier = TMS_MODIFIER_PORT_AT;
                  cmd.size++;
                  break;
                case o_reg:
                  cmd.Operands[n].tms_operator1 |= TMS_OPERATOR_PORT & 0xFF;
                  cmd.Operands[n].tms_operator2 |= (TMS_OPERATOR_PORT >> 8);
                  cmd.size++;
                  break;
              }
            }
          ...
          case BYTE_LR:
          case BYTE_CR: // .lr & .cr
            cmd.SpecialModes |= nextbyte == BYTE_LR ? TMS_MODE_LR : TMS_MODE_CR;
            cmd.size++;
            break;
        }
      }
  ...
  if ( cmd.itype == TMS320C55_null ) return 0;
  return cmd.size;
}
