...
static void handle_operand(op_t &op, bool loading) {
    switch (op.type) {
        ...
        case o_displ:
            handle_imm();
            // if the value was converted to an offset, then create a data xref:
            if (isOff(uFlag, op.n)) ua_add_off_drefs(op, loading ? dr_R : dr_W);
