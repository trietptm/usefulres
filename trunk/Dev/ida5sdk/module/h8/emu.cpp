...
{
  return (x.type != o_displ || x.type != o_phrase) && issp(x.phrase);
}

//----------------------------------------------------------------------
static void add_stkpnt(int value)
{
  if ( value & 1 ) value++;
  add_auto_stkpnt(cmd.ea+cmd.size, value);
}
...
static void trace_sp(void)
{
  ...
  {
    int size = get_dtyp_size(cmd.Op2.dtyp);
    if ( cmd.Op2.type == o_reglist ) size *= cmd.Op2.nregs;
    add_stkpnt(size);
    return;
  }
  ...
  {
    int size = get_dtyp_size(cmd.Op1.dtyp);
    if ( cmd.Op1.type == o_reglist ) size *= cmd.Op1.nregs;
    add_stkpnt(-size);
    return;
  }
  ...
  switch ( cmd.itype )
  {
    case H8_add:
    case H8_adds:
      if ( !issp(cmd.Op2.reg) ) return;
      if ( get_op_value(cmd.Op1, &v) ) add_stkpnt(v);
      break;
    case H8_sub:
    case H8_subs:
      if ( !issp(cmd.Op2.reg) ) return;
  ...
  switch ( x.type )
  {
    ...
    case o_displ:
      ...
      if ( may_create_stkvars() && !isDefArg(uFlag, x.n) )
      {
        ...
        {
          if ( ua_stkvar(x, x.addr) ) op_stkvar(cmd.ea, x.n);
        }
      }
      break;
    case o_near:
...
bool create_func_frame(func_t *pfn)
{
  ...
  if ( pfn->frame == BADNODE )
  {
    ...
    {
      setflag((ulong &)pfn->flags,FUNC_FRAME,bpused);
      return add_frame(pfn, frsize, regs, argsize);
    }
  }
  return code;
}
...
static bool is_jump_pattern1(ea_t *base, ea_t *table, ea_t *defea, int *size, int *elsize)
{
  ...
  int baseh = cmd.Op1.value;       // msb of base
  ea_t eah = cmd.ea;
  ...
  int basel = cmd.Op1.value;       // lsb of base
  ea_t eal = cmd.ea;
...
static bool check_for_table_jump(void)
{
  ...
  for ( i=0; i < size; i++ )
  {
    ...
    if ( isTail(F2)
      || isData(F2)
      || (!isCode(F2) && !ua_ana0(base+el)) ) break;
  }
  cmd = saved;
  size = i;
  if ( size != oldsize )
