...
inline ulong code_address(signed int delta)
{
  ulong newip = cmd.ip + 1 + delta;
...
{
  return get_full_byte(cmd.ea + cmd.size++);
}

//--------------------------------------------------------------------------
inline void opreg(op_t &x, int n)
{
  x.type = o_reg;
  x.dtyp = dt_byte;
  x.reg  = n;
