...
static char *BitNamesCPU[] = { "IPL", "N", "V", "m", "x", "D", "I", "Z", "C" };

static char *BitNamesPUL[] = { "PS", "0", "DT", "DP0", "Y", "X", "B", "A" };
...
inline void GetNumPUL(ulong data)
{
  int bitOut=0;
  for(int i=0; i<8; i++)
  {
...
inline void GetNumDPRn(ulong data)
{
   char tmp[2];
...
inline void GetCLPFlags(ulong data)
{
  int bitOut=0;
  for(int i=0; i<8;i++)
  {
...
static int near getNumDPR(int iDPR )
{
...
static void near OutDPRReg(ea_t &Addr, int gDPReg)
{
  if( gDPReg == 1)
   {
      char szTemp[5];
...
static ulong near GetValueDP(int DPR )
{
  if( getDPReg == 1)
  {
    switch(DPR)
...
static void near OutDPR( ulong Data )
{
   char tmp[20];
   ...
   out_symbol(':');
   qsnprintf(tmp, sizeof(tmp), "%X", Val+GetValueDP( Data&0xC0 ) );
   out_line(tmp, COLOR_NUMBER);
}

static void near OutDT( ulong Data )
{
   char tmp[20];
...
static void near OutIMM(ulong Data )
{
   char tmp[20];
...
static char* near GetPrefics( int Raz)
{
