...
static const predefined_t *GetPredefined(predefined_t *ptr,ulong addr)
{
...
static ulong AdditionalSegment(int size,int offset,char *name)
{
  segment_t s;
...
{ // Various messages:
  ...
  case IDP_NEWFILE:
	...
	if ( sptr->startEA-get_segm_base(sptr) == 0 ) {
      ...
      for(ulong ea=0;ea<0x80;ea++)doByte(ea,1);
      const predefined_t *ptr;
      // создадим все регистры в sfr'e и распишем все данные
      for ( ptr=iregs; ptr->name != NULL; ptr++ ){
		ulong ea = ptr->addr;
		ulong oldea = get_name_ea(ptr->name);
		if ( oldea != ea ) {
...
static char *shnames[] = { "TLCS900", NULL };
// длинные имена процессоров
static char *lnames[] = { "TLCS900", NULL };
