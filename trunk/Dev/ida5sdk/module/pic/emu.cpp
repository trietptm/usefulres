...
bool is_pcl(void)
{
  if ( cmd.Op1.type == o_mem )
  {
    ...
    {
      case PIC12:
      case PIC14: return (cmd.Op1.addr & 0x7F) == 0x2;
...
bool is_bank(void)
{
  if ( cmd.Op1.type == o_mem )
  {
    ...
    {
      case PIC12:
      case PIC14: return (cmd.Op1.addr & 0x7F) == 0x3;
...
bool is_pclath(void)
{
  if ( cmd.Op1.type == o_mem )
  {
    switch ( ptype )
    {
      ...
      case PIC14: return (cmd.Op1.addr & 0x7F) == 0xA;
        ...
        if ( may_create_stkvars())
        {
          ...
          else if ( x.addr == PIC16_PLUSW2 )
          {
            ...
            if ( decode_prev_insn(cmd.ea) != BADADDR && cmd.itype == PIC_movlw )
            {
              ...
              if ( pfn != NULL && (pfn->flags & FUNC_FRAME) != 0 )
              {
                if ( ua_stkvar(cmd.Op1, cmd.Op1.value) )
...
//----------------------------------------------------------------------
int PIC_get_frame_retsize(func_t *)
{
  return 0;
}
