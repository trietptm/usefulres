...
static int op_rd(op_t &x, int reg, int dtyp)
{
  x.type = o_reg;
  x.dtyp = dtyp;
  x.reg  = (dtyp == dt_byte ? rR0L : rR0) + reg;
  return 1;
}

//----------------------------------------------------------------------
// register indirect
static int op_ph(op_t &x, int phrase, int reg, int dtyp)
{
  x.type = o_phrase;
  x.dtyp = dtyp;
...
static int op_ds(op_t &x, int phrase, int ireg, sval_t disp, int dtyp)
{
  if (disp > 0xFD00L)
  {
        ...
        switch (code)
        {
          ...
          case 0x48: // LEA
            ...
            if (code & 0x8) // 16bit offset
              op_ds(cmd.Op2, fRid16, b1&0x07, (long)ua_next_word_be(), dt_word);
            else
