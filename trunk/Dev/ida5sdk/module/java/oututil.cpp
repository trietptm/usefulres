...
int check_special_label(char buf[MAXSTR], register int len)
{
  if(   (unsigned)len >= sizeof("met000_end")-1
     && (*(ulong*)buf & 0xFFFFFF) == ('m'|('e'<<8)|('t'<<16))) {

    switch(*(ulong*)&buf[len -= 4]) {
...
//----------------------------------------------------------------------
bool block_end(size_t off)
{
  return(MakeLine(COLSTR("}", SCOLOR_SYMBOL), off));
}

//----------------------------------------------------------------------
bool block_close(size_t off, const char *name)
{
...
bad:
  ...
  for(register unsigned i = 0; ;  ) {
    ...
    if(out_alt_ind(node.altval(++i))) goto bad;
    if(i >= cnt) return(1);
...
//--------------------------------------------------------------------------
uchar putScope(ushort scope, ulong doff)
{
