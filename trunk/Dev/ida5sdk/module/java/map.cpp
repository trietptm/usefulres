...
set_max:
  ...
  do {
    ...
    for(tflag = unsort, pos = 10, i = 1; i <= curClass.maxCPindex; i++) {
...
DEB_ASSERT((i == curClass.maxCPindex), "map:CPend");
  ...
  if(numstr) {  // if error print before - header problems!
    ...
    if((encinc & 2) && curClass.msgNode) {
      ...
      int slen = print_loader_messages(str, NULL);
...
#error
#endif
  {
    ...
    if(!AskUsingForm_c(fmt, &maxv, &minv, &jdk, &rtyp, &mod)) qexit(1);
  }
