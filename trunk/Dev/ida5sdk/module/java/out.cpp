...
//----------------------------------------------------------------------
static uchar OutModes(ulong mode)
#define OA_THIS   0
#define OA_FIELD  1
#define OA_METHOD 2
...
//--------------------------------------------------------------------------
static bool close_annotation(size_t pos)
{
...
bad:
    ...
    x.addr =  co.value2;
#ifdef __EA64__
    x.value = make_ulonglong(long(x.value), long(x.addr));
#endif
...
{
  netnode temp(node);
  ulong   len, vid, cnt = temp.altval(0);
  ...
  do {
    ...
    len = temp.supstr(cnt, &fnm[1], sizeof(fnm)-3);
    if(!len) goto destr;
    fnm[++len] = '"';
    fnm[++len] = '\0';
    char *pf = fnm;
    ...
    if((vid = temp.altval(cnt)) == 0 || vid > curClass.maxCPindex) goto destr;
    out_tagon(color);
  ...
  switch(str[0] = getMySeg(ea-1)->type) { // also set curSeg
    case SEG_CODE:
      ...
      if(curSeg.excNode) {
        ...
        if((j = enode.altval(0)) == 0) DESTROYED("out::segend");
...
illcall:
    ...
    case SEG_BSS:
      ...
      lvc = 0;  // unification
      off = ea - curSeg.DataBase;
      if((ulong)off >= (ulong)curSeg.DataSize) off = (asize_t)-1;
      else if(   curSeg.varNode
              && (lvc = netnode(curSeg.varNode).altval(off)) != 0) {
        if((long)lvc < 0) {
          lvc = -(long)lvc;
    ...
    case SEG_XTRN:
      if(ip > (ulong)curClass.xtrnCnt) goto stop;
      if(sz) goto illcall;
      if(!ip) {
...
//         && !jasmin()) out_line(".*", COLOR_SYMBOL);
    ...
    case SEG_IMP:
      if(ip > (ulong)curClass.FieldCnt) goto stop;
      if(sz) goto illcall;
      gl_name = gl_comm = gl_xref = 0;
      if(!ip) {
        ...
        if(curClass.innerNode) {
          ...
          if((j = inode.altval(0)) == 0) goto do_destroy;
          ...
          do {
            ...
            if(OutModes((((ulong)ic.access) << 16) | OA_NEST)) break;
            if(ic.name) {
      ...
      if(curField.valNode) {
        ...
        if((valcnt = vnode.altval(0)) == 0) goto do_destroy;
        x.n = 0;
        x.flags = OF_SHOW;
        x.type = o_imm;
