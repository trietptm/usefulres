...
load2:
      ...
      cmd.Op1.value = cntopis.value;     // for string index to Utf8
#ifdef __EA64__
      cmd.Op1.value = make_ulonglong(long(cmd.Op1.value), ulong(cmd.Op1.addr));
#endif
      return 1;
...
methodchk:
    ...
    case C_Type:
      ...
      cmd.Op1.addr = ((ulong)fmt_dscr << 16) | (ushort)fmt_classname;
      goto loadref1; //load 1 ind.

    case C_TypeName:
      if(i != CONSTANT_Class) break;
      if(!(cntopis.flag & (HAS_TYPEDSCR | HAS_CLSNAME))) goto wrnret;
      cmd.Op1.addr = ((ulong)fmt_cast << 16) | (ushort)
    ...
    case j_lookupswitch:
      {
        ...
        cmd.swit = 1;
        for(top = (4  - ((cmd.ip + cmd.size) % 4)) & 3; top; top--)
          if(ua_next_byte()) {
            if(!debugmode) return(0);
            cmd.swit |= 0100;
          }
        cmd.Op3.type = o_near;
        cmd.Op3.offb = cmd.size;
        ...
        if(cmd.itype == j_tableswitch) {
          ...
          count = ((cmd.Op2.value = ua_next_long()) - count + 1);
        }
        cmd.Op3.value = count;
        cmd.Op2.addr = cmd.ip + cmd.size;
        top = curSeg.CodeSize - cmd.ip;
        while(count--) {
          ...
          if((ulong)cmd.size >= top) return(0);
        }
      }
      break;
