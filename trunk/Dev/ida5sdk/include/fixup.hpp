...
idaman void ida_export set_fixup(ea_t source, fixup_data_t *fp);
...
inline void del_fixup(ea_t source)
{
  fixup_node.supdel(source);
}


// Get fixup information

inline bool get_fixup(ea_t source, fixup_data_t *fd)
{
  return fixup_node.supval(source, fd, sizeof(*fd)) > 0;
}


// Enumerate addresses with fixup information:

inline ea_t get_first_fixup_ea(void)   { return fixup_node.sup1st(); }
inline ea_t get_next_fixup_ea(ea_t ea) { return fixup_node.supnxt(ea); }
inline ea_t get_prev_fixup_ea(ea_t ea) { return fixup_node.supprev(ea); }
