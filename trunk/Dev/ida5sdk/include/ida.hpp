...
typedef ulong flags_t;    // 32-bit flags for each address
typedef ea_t tid_t;       // type id (for enums, structs, etc)

typedef ulong bgcolor_t;        // Background color in RGB
...
#define DEMNAM_GCC3  4                  // assume gcc3 names (valid for gnu compiler)
  uchar get_demname_form(void) const { return demnames & DEMNAM_MASK; }
...
#define LN_WEAK         0x08            //   weak names
  ...
  uval_t        short_demnames;         // short form of demangled names
  uval_t        long_demnames;          // long form of demangled names
                                        // see demangle.h for definitions
  uval_t        datatypes;              // data types allowed in data carousel
                                        // used in MakeData command.
  uval_t        strtype;                // current ascii string type
...
inline void idaapi setflag(uchar &where,uchar bit,int value)
  {
     if ( value ) where |= bit;
     else where &= uchar(~bit);
  }


inline void idaapi setflag(ushort &where,ushort bit,int value)
  {
     if ( value ) where |= bit;
     else where &= ushort(~bit);
  }

inline void idaapi setflag(ulong &where,ulong bit,int value)
  {
     if ( value ) where |= bit;
     else where &= ~bit;
  }
...
// under windows
idaman char *ida_export ansi2idb(char *name);
...
inline void dual_text_options_t::copy_to_inf(idainfo &inf) const { dto_copy_to_inf(this, &inf); }
inline void dual_text_options_t::copy_from_inf(const idainfo &inf) { dto_copy_from_inf(this, &inf); }
