...
//------------------------------------------------------------------------
// A function is a continuous range of addresses with characteristics:
...
#define FUNC_TAIL       0x00008000L     // This is a function tail.
                                        // Other bits should be clear (except FUNC_HIDDEN)
...
idaman int ida_export get_func_bitness(func_t *pfn);
...
// The loop may continue untel the next() or prev() function returns false.
...
public:
  ...
  func_tail_iterator_t(func_t *pfn, ea_t ea=BADADDR) { set(pfn, ea); }
  bool set(func_t *pfn, ea_t ea=BADADDR) { return func_tail_iterator_set(this, pfn, ea); }
  ...
  const area_t &chunk(void) const { if ( pfn == NULL ) return seglim; /*keep in cache*/ get_fchunk(pfn->startEA); return idx >= 0 ? pfn->tails[idx] : *(area_t*)pfn; }
  ...
  void reset_func(func_t *_pfn) { pfn = _pfn; } // for internal use only!
};
...
public:
  ...
  func_item_iterator_t(func_t *pfn, ea_t ea=BADADDR) { set(pfn, ea); }
  ...
  void reset_func(func_t *pfn) { fti.reset_func(pfn); }
  // set an arbitrary range
  ...
  bool next_addr(void) { return next(NULL, NULL); }
  ...
  bool prev_addr(void) { return prev(NULL, NULL); }
...
public:
  ...
  func_parent_iterator_t(func_t *fnt) { set(fnt); }
  bool set(func_t *fnt) { return func_parent_iterator_set(this, fnt); }
  ea_t parent(void) const { /*keep in cache*/ get_fchunk(fnt->startEA); return fnt->referers[idx]; }
...
idaman long ida_export get_idasgn_desc(int n,
                                       char *signame,
                                       size_t signamesize,
                                       char *optlibs,
                                       size_t optlibssize);
...
idaman char *ida_export get_sig_filename(char *buf,
                                         size_t bufsize,
                                         const char *signame);
...
idaman char *ida_export get_idasgn_title(const char *name,
                                         char *buf,
                                         size_t bufsize);
