...
function favorBox(action) {
	...
	if(action == 'open') {
		...
		if(!$('favorlayer')) {
			...
			$('favorlayer').innerHTML = '<div style="width: 396px; background: #666666; margin: 5px auto; text-align: left">' +
				'<div style="width: ' + favorwidth + 'px; height: ' + favorheight + 'px; padding: 1px; background: #FFFFFF; border: 1px solid #7597B8; position: relative; left: -3px; top: -3px">' +
				...
				'<a href="javascript:favorBox(\'close\');" target="_self"><img style="position: absolute; right: 10px; top: 15px" src="' + favorcloseimg +'" title="Close" border="0" /></a>' +
		...
		$('favorlayer').style.display = '';
		$('favorlayer').style.top = ((clientHeight - favorheight) / 2 + scrollTop) + 'px';
...
function favorDrag(obj) {
	obj.onmousedown = function(e) {
		...
		favordragstart[2] = parseInt($('favorlayer').style.left);
		favordragstart[3] = parseInt($('favorlayer').style.top);
		$('favormask').style.display = '';
		favordoane(e);
	}
